...
def unify_input_heights():
    ...
    # 统一的输入框高度样式
    input_pattern = re.compile(r'(<input[^>]+class="[^>]*)(w-full\s+px-3\s+py-\d+)([^>]*")', re.DOTALL)
    select_pattern = re.compile(r'(<select[^>]+class="[^>]*)(w-full\s+px-3\s+py-\d+)([^>]*")', re.DOTALL)
    ...
    for file_path in files:
        ...
        try:
            ...
            # 统一输入框高度
            content = input_pattern.sub(r'\1w-full px-3 py-2\3', content)
            
            # 统一下拉选择框高度
            content = select_pattern.sub(r'\1w-full px-3 py-2\3', content)
