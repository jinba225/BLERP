...
class PurchaseSyncService:
    ...
    def link_purchase_order_to_platform(self, purchase_order_id: int, platform_id: int, 
                                     platform_account_id: int, mappings: list) -> dict:
        """关联采购订单商品到平台商品
        ...
        for platform_map in platform_maps:
            try:
                ...
                adapter.update_product(
                    platform_map.platform_product_id,
                    product_data
                )
    ...
    def process_purchase_sync_queue(self, limit: int = 100) -> dict:
        ...
        # 获取待处理的同步任务
        sync_queues = PurchaseSyncQueue.objects.filter(
            status='pending'
        )[:limit]
    ...
    def auto_restock_alert(self) -> list:
        ...
        for stock in low_stock_stocks:
            ...
            has_pending_purchase = PurchaseOrderItem.objects.filter(
                ...
                purchase_order__status__in=['draft', 'approved', 'partial_received', 'fully_received']
            ).exists()
    ...
    def _build_sync_data(self, order_item: PurchaseOrderItem) -> dict:
        ...
        """
        return {
            'platform_product_id': order_item.platform_maps.first().platform_product_id if order_item.platform_maps.exists() else '',
            'product_data': {
    ...
    def _get_product_images(self, product) -> list:
        ...
        if product and hasattr(product, 'images'):
            images = [
                image.image_url 
                for image in product.images.all()[:5]  # 最多5张图片
            ]
        return images
