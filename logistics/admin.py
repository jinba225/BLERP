...
from logistics.models import LogisticsCompany, ShippingOrder, TrackingInfo, LogisticsCost, WaybillTemplate
...
class LogisticsCompanyAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('基本信息', {
            'fields': ('name', 'code', 'tier', 'is_active')
        }),
        ('API配置', {
...
class ShippingOrderAdmin(admin.ModelAdmin):
    ...
    list_display = ['tracking_number', 'platform_order', 'logistics_company', 
    ...
    fieldsets = (
        ...
        ('物流信息', {
            'fields': ('shipping_status', 'shipping_cost', 'weight', 'volume',
                      'shipped_at', 'delivered_at', 'last_track_at')
        }),
        ('面单信息', {
            'fields': ('waybill_image', 'waybill_pdf'),
            'classes': ('collapse',)
        }),
        ('其他', {
            'fields': ('note',),
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related(
            'platform_order', 'logistics_company'
        )
...
class TrackingInfoAdmin(admin.ModelAdmin):
    ...
    list_display = ['shipping_order', 'track_time', 'track_status', 
                    'track_location', 'operator']
...
class LogisticsCostAdmin(admin.ModelAdmin):
    ...
    list_display = ['shipping_order', 'cost_type', 'cost_amount', 
                    'cost_date', 'description']
...
class WaybillTemplateAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('模板内容', {
            'fields': ('template_html', 'css_style')
        }),
        ('其他', {
            'fields': ('thumbnail',),
            'classes': ('collapse',)
