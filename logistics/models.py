...
class LogisticsCompany(BaseModel):
    ...
    code = models.CharField('物流公司代码', max_length=50, unique=True,
                           help_text='唯一标识，如SF、YTO、DHL等')
    api_endpoint = models.URLField('API端点', blank=True, max_length=500,
                                  help_text='官方API地址')
    tracking_url_template = models.CharField('查询URL模板', max_length=500, blank=True,
                                         help_text='物流查询URL模板，如https://www.sf-express.com/waybill/#search/all/{tracking_number}')
    tier = models.CharField('支持级别', max_length=10, choices=TIER_CHOICES, default='tier_1',
                          help_text='API支持级别')
...
class ShippingOrder(BaseModel):
    ...
    shipping_status = models.CharField('物流状态', max_length=20, choices=SHIPPING_STATUS_CHOICES,
                                     default='pending')
    shipping_cost = models.DecimalField('物流费用', max_digits=10, decimal_places=2,
                                       default=0, help_text='物流总费用')
    weight = models.DecimalField('重量(kg)', max_digits=10, decimal_places=3,
                              null=True, blank=True, help_text='订单总重量')
    volume = models.DecimalField('体积(m³)', max_digits=10, decimal_places=3,
                              null=True, blank=True, help_text='订单总体积')
    ...
    waybill_image = models.ImageField('面单图片', upload_to='waybills/%Y/%m/',
                                    null=True, blank=True)
    waybill_pdf = models.FileField('面单PDF', upload_to='waybills/pdf/%Y/%m/',
                                  null=True, blank=True)
...
class LogisticsCost(BaseModel):
    ...
    reference_number = models.CharField('参考号', max_length=200, blank=True,
...
class WaybillTemplate(BaseModel):
    ...
    thumbnail = models.ImageField('缩略图', upload_to='waybill_templates/thumbnails/',
                               null=True, blank=True)
