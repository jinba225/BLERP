...
class RealtimeDashboard(BaseModel):
    ...
    theme = models.CharField('主题', max_length=50, default='default', 
                               help_text='default, dark, blue, green')
    ...
    # 创建者信息
    created_by = models.ForeignKey('users.User', on_delete=models.CASCADE, related_name='created_realtime_dashboards', verbose_name='创建者')
...
class RealtimeWidget(BaseModel):
    ...
    dashboard = models.ForeignKey(RealtimeDashboard, on_delete=models.CASCADE, related_name='widgets', verbose_name='大屏')
    ...
    # 数据源配置
    data_source = models.CharField('数据源', max_length=200, help_text='数据源类路径，如bi.services.realtimedata.RealtimeDataService')
    ...
    # 布局配置
    position = models.CharField('位置', max_length=100, blank=True, help_text='大屏中的位置，如 top-left, top-center')
...
class RealtimeMetric(BaseModel):
    ...
    widget = models.OneToOneField(RealtimeWidget, on_delete=models.CASCADE, related_name='metric', verbose_name='组件')
    ...
    metric_format = models.CharField('格式', max_length=50, default='{value}',
                             help_text='数值格式，如{value}、{value,000}、{value:00:00}')
    
    # 阈值配置
    target_value = models.DecimalField('目标值', max_digits=10, decimal_places=2, null=True, blank=True)
    warning_threshold = models.DecimalField('警告阈值', max_digits=10, decimal_places=2, null=True, blank=True)
    danger_threshold = models.DecimalField('危险阈值', max_digits=10, decimal_places=2, null=True, blank=True)
    
    # 趋势配置
    show_trend = models.BooleanField('显示趋势', default=True)
    trend_period = models.CharField('趋势周期', max_length=20, default='24h', 
...
class DataDrilldown(BaseModel):
    ...
    report = models.ForeignKey('bi.Report', on_delete=models.CASCADE, related_name='drilldowns', verbose_name='报表')
