...
class Report(BaseModel):
    ...
    platform = models.ForeignKey(Platform, on_delete=models.CASCADE, null=True, blank=True, related_name='reports', verbose_name='平台')
    platform_account = models.ForeignKey(PlatformAccount, on_delete=models.CASCADE, null=True, blank=True, related_name='reports', verbose_name='平台账号')
    created_by = models.ForeignKey('users.User', on_delete=models.SET_NULL, null=True, blank=True, related_name='created_reports', verbose_name='创建人')
...
class ReportData(BaseModel):
    ...
    report = models.ForeignKey(Report, on_delete=models.CASCADE, related_name='data_records', verbose_name='报表')
...
class SalesSummary(BaseModel):
    ...
    platform = models.ForeignKey(Platform, on_delete=models.CASCADE, related_name='sales_summaries', verbose_name='平台')
    platform_account = models.ForeignKey(PlatformAccount, on_delete=models.CASCADE, null=True, blank=True, related_name='sales_summaries', verbose_name='平台账号')
    ...
    # 转化指标
    conversion_rate = models.DecimalField('转化率', max_digits=5, decimal_places=2, default=0, help_text='百分比')
    repeat_purchase_rate = models.DecimalField('复购率', max_digits=5, decimal_places=2, default=0, help_text='百分比')
...
class ProductSales(BaseModel):
    ...
    product = models.ForeignKey(Product, on_delete=models.CASCADE, related_name='sales_data', verbose_name='商品')
    platform = models.ForeignKey(Platform, on_delete=models.CASCADE, related_name='product_sales', verbose_name='平台')
...
class InventoryAnalysis(BaseModel):
    ...
    product = models.ForeignKey(Product, on_delete=models.CASCADE, related_name='inventory_analysis', verbose_name='商品')
    shop = models.ForeignKey(Shop, on_delete=models.CASCADE, related_name='inventory_analysis', verbose_name='仓库')
    platform = models.ForeignKey(Platform, on_delete=models.CASCADE, null=True, blank=True, related_name='inventory_analysis', verbose_name='平台')
    ...
    # 预警指标
    stock_status = models.CharField('库存状态', max_length=20, default='normal',
...
class PlatformComparison(BaseModel):
    ...
    platform = models.ForeignKey(Platform, on_delete=models.CASCADE, related_name='comparison_data', verbose_name='平台')
    platform_account = models.ForeignKey(PlatformAccount, on_delete=models.CASCADE, null=True, blank=True, related_name='comparison_data', verbose_name='平台账号')
...
class Dashboard(BaseModel):
    ...
    description = models.TextField('描述', blank=True)
    created_by = models.ForeignKey('users.User', on_delete=models.CASCADE, related_name='dashboards', verbose_name='创建人')
    
    # 布局配置
    layout = models.JSONField('布局配置', default=dict, blank=True, help_text='组件布局')
    widgets = models.ManyToManyField(DashboardWidget, through='DashboardWidgetConfig', related_name='dashboards', verbose_name='组件')
    
    # 访问控制
    is_public = models.BooleanField('是否公开', default=False, help_text='公开仪表盘对所有用户可见')
    allowed_users = models.ManyToManyField('users.User', blank=True, related_name='accessible_dashboards', verbose_name='允许访问的用户')
...
class DashboardWidgetConfig(BaseModel):
    ...
    widget = models.ForeignKey(DashboardWidget, on_delete=models.CASCADE, related_name='dashboard_configs', verbose_name='组件')
