...
class SalesReportService:
    ...
    def generate_daily_sales_summary(self, platform_id: Optional[int] = None,
                                     platform_account_id: Optional[int] = None,
                                     report_date: date = None) -> SalesSummary:
        """
        生成日销售汇总
        ...
        # 查询平台订单
        queryset = PlatformOrder.objects.filter(
        ...
        avg_order_value = total_amount_value / total_orders_count if total_orders_count > 0 else Decimal('0')
    ...
    def generate_product_sales_report(self, platform_id: Optional[int] = None,
                                       platform_account_id: Optional[int] = None,
                                       report_date: date = None) -> List[ProductSales]:
        """
        生成商品销售报表
        ...
        # 查询订单商品
        queryset = PlatformOrderItem.objects.filter(
    ...
    def _calculate_return_rate(self, queryset, product) -> Decimal:
        ...
        # 假设退款订单为退货（简化处理）
        refunded_count = queryset.filter(
            product=product,
...
class InventoryReportService:
    ...
    def generate_inventory_analysis(self, shop_id: int, product_id: Optional[int] = None) -> List[InventoryAnalysis]:
        """
        生成库存分析
    ...
    def _determine_stock_status(self, current_stock: int, safety_stock: int, days_of_stock: int) -> str:
        """确定库存状态"""
        if current_stock == 0:
            return 'out'
...
class PlatformComparisonService:
    ...
    def generate_platform_comparison(self, report_date: date = None,
                                     report_period: str = 'daily') -> List[PlatformComparison]:
        """
        生成平台对比数据
        ...
        for platform in platforms:
            ...
            for account in platform_accounts:
                ...
                if report_period == 'daily':
                    ...
                    queryset = queryset.filter(created_at__date__gte=week_start, created_at__date__lte=report_date)
                elif report_period == 'monthly':
                    month_start = report_date.replace(day=1)
                    queryset = queryset.filter(created_at__date__gte=month_start, created_at__date__lte=report_date)
                ...
                avg_order_value = (data['sales_amount'] or Decimal('0')) / (data['order_count'] or 1)
    ...
    def _calculate_growth_rate(self, platform, account, metric_type: str,
                               current_date: date, period: str) -> Decimal:
        """计算增长率"""
        # 获取上期数据
        if period == 'daily':
        ...
        try:
            ...
            if metric_type == 'order':
                ...
                current_value = current_queryset.aggregate(total=Sum('amount'))['total'] or Decimal('0')
                previous_value = previous_queryset.aggregate(total=Sum('amount'))['total'] or Decimal('0')
            else:
                return Decimal('0')
    ...
    def _calculate_rankings(self, comparison_list: List[PlatformComparison], report_date: date, period: str):
        """计算排名"""
        # 按销售金额排名
...
class ReportGenerator:
    ...
    def generate_all_daily_reports(self, report_date: date = None):
        ...
        for shop in shops:
            inventory_list = self.inventory_service.generate_inventory_analysis(
                shop_id=shop.id
            )
