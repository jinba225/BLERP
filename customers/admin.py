...
from .models import (
    CustomerCategory, Customer, CustomerContact, CustomerAddress,
    CustomerCreditHistory, CustomerVisit
)
...
class CustomerCategoryAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('基本信息', {
            'fields': ('name', 'code', 'description', 'discount_rate')
        }),
        ('状态', {
            'fields': ('is_active',)
        }),
...
class CustomerAdmin(ImportExportModelAdmin):
    ...
    list_display = [
        'code', 'name', 'customer_level', 'status',
        'sales_rep', 'credit_limit', 'created_at'
    ]
    ...
    fieldsets = (
        ...
        ('业务信息', {
            'fields': ('industry', 'business_license', 'tax_number', 'bank_name', 'bank_account'),
            'classes': ('collapse',)
        }),
        ('销售信息', {
...
class CustomerContactAdmin(ImportExportModelAdmin):
    ...
    list_display = ['customer', 'name', 'position', 'department', 'phone', 'mobile', 'email', 'is_primary']
...
class CustomerCreditHistoryAdmin(admin.ModelAdmin):
    """客户信用历史管理"""
    list_display = ['customer', 'credit_type', 'old_limit', 'new_limit', 'approved_by', 'created_at']
    ...
    fieldsets = (
        ('基本信息', {
            'fields': ('customer', 'credit_type')
        }),
        ('额度变更', {
            'fields': ('old_limit', 'new_limit', 'reason')
        }),
        ('审批信息', {
            'fields': ('approved_by',)
        }),
...
class CustomerVisitAdmin(admin.ModelAdmin):
    """客户拜访记录管理"""
    list_display = [
        'customer', 'visit_type', 'visit_purpose', 'visit_date',
        'visitor', 'contact_person', 'next_visit_date'
    ]
    ...
    fieldsets = (
        ('基本信息', {
            'fields': ('customer', 'visit_type', 'visit_purpose', 'visit_date', 'visitor', 'contact_person')
        }),
        ('拜访内容', {
