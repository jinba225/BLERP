...
def product_listings_fragment(request, product_id):
    ...
    try:
        ...
        ).select_related('platform', 'account')
        ...
        data = [{
            ...
            'last_synced_at': l.last_synced_at.strftime('%Y-%m-%d %H:%M:%S') if l.last_synced_at else None,
...
def publish_listing(request):
    ...
    try:
        ...
        if result['success'] > 0:
            return JsonResponse({
                'success': True,
                'message': f'成功发布{result["success"]}个Listing'
            })
        else:
            return JsonResponse({
                'success': False,
                'message': '发布失败：' + str(result['errors'])
            })
    except Exception as e:
        return JsonResponse({'success': False, 'message': str(e)}, status=500)
...
def listings_list(request):
    ...
    """
    try:
        listings = ProductListing.objects.select_related(
