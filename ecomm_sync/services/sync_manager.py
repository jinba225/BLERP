...
from ecomm_sync.models import (
    EcommProduct, ProductChangeLog, SyncLog,
    EcommPlatform
)
...
class ChangeDetector:
    ...
    def detect_changes(
        ...
        if abs(new_price - old_price) > 0.1:
            changes.append(ProductChangeLog(
            ...
            logger.info(
                f'价格变更: {product.code} ¥{old_price} -> ¥{new_price}'
            )
        ...
        if old_stock != new_stock:
            ...
            logger.info(
                f'库存变更: {product.code} {old_stock} -> {new_stock}'
            )
        ...
        if old_status != new_status:
            ...
            logger.info(
                f'状态变更: {product.code} {old_status} -> {new_status}'
            )
...
class SyncManager:
    ...
    async def sync_platform_products(
        ...
        try:
            ...
            for ecomm_product in ecomm_products:
                try:
                    latest_data = await scraper.scrape_product(
                        ecomm_product.external_id
                    )
                    ...
                    logger.error(
                        f'同步失败: {ecomm_product.external_id}, 错误: {e}'
                    )
                    results['failed'] += 1
                    results['errors'].append({
                        'external_id': ecomm_product.external_id,
                        'error': str(e)
                    })
    ...
    async def sync_price_changes(self, platform: EcommPlatform) -> dict:
        ...
        try:
            ...
            logger.info(
                f'价格监控完成: 检测到 {changes_detected} 个价格变化, '
                f'耗时 {execution_time:.1f}秒'
            )
            
            return {
                'total': ecomm_products.count(),
                'price_changes': changes_detected
            }
