...
class SyncLogAdmin(admin.ModelAdmin):
    """同步日志管理"""
    list_display = ['log_type', 'platform', 'status', 'records_processed', 'records_succeeded', 'execution_time', 'created_at']
...
class PlatformAccountAdmin(admin.ModelAdmin):
    """平台账号管理"""
    list_display = ['account_name', 'account_type', 'platform_name', 'is_active', 
...
class ProductListingAdmin(admin.ModelAdmin):
        ...
        for listing in queryset:
            try:
                ...
                if stock:
                    ...
                    success = service.update_listing_price(listing.id, stock.selling_price)
                    if success:
                        count += 1
    ...
    def update_stock_from_erp(self, request, queryset):
        ...
        for listing in queryset:
            try:
                ...
                if stock:
                    service = ListingService()
                    success = service.update_listing_stock(listing.id, stock.qty_in_stock)
                    if success:
                        count += 1
            except Exception as e:
                self.message_user(request, f"Listing {listing.id} 更新失败: {e}")
    ...
    def sync_to_platform(self, request, queryset):
        ...
        for listing in queryset:
            try:
                service = ListingService()
                success = service.sync_product_listings(listing.product.id)
                if success:
                    count += 1
            except Exception as e:
    ...
    def batch_delist(self, request, queryset):
        ...
        for listing in queryset:
            try:
                service = ListingService()
                success = service.delete_listing(listing.id)
                if success:
                    count += 1
            except Exception as e:
...
class APICallLogAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('响应信息', {
            'fields': ('response_status', 'response_data', 'success', 'execution_time')
        }),
        ('错误信息', {
