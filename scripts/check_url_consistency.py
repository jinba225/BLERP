...
class URLConsistencyChecker:
    ...
    def _find_unused_urls(self):
        ...
        for defined_url in self.url_extractor.defined_urls:
            ...
            if ':' in defined_url:
                ...
                if (defined_url in used_names or
                    name in used_names or
                    any(defined_url in ref for ref in self.template_extractor.template_urls.keys())):
                    is_used = True
            else:
    ...
    def _generate_json_report(self):
        ...
        for item in self.results['undefined_in_urls']:
            references = [
                {
                    'file': str(Path(ref[0]).relative_to(project_root)),
                    'line': ref[1]
                }
                for ref in item['references']
            ]
            report['undefined_urls'].append({
                'name': item['name'],
                'references': references
            })
