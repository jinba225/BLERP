...
from finance.models import SupplierPrepayment, CustomerPrepayment
...
def test_supplier_prepayment_consolidation():
    ...
    # 更新原记录状态
    updated_count = active_prepays.update(
        status='merged',
        merged_into=consolidated
    )
    ...
    for prepay in new_active_prepays:
        print(f'    ID:{prepay.id} | 金额:{prepay.amount} | 余额:{prepay.balance} | 合并记录:{prepay.is_consolidated}')
...
def test_customer_prepayment_consolidation():
    ...
    for prepay in active_prepays:
        total_amount += prepay.amount
        total_balance += prepay.balance
        print(f'  ID:{prepay.id} | {prepay.received_date} | 金额:{prepay.amount} | 余额:{prepay.balance}')
