...
def fix_svg_attributes(file_path, dry_run=False):
    ...
    try:
        ...
        def fix_svg_tag(match):
            ...
            # 如果已经有所有必需属性，跳过
            if (
                ('fill="none"' in svg_tag or "fill='none'" in svg_tag)
                and ('viewBox="0 0 24 24"' in svg_tag or "viewBox='0 0 24 24'" in svg_tag)
                and ('stroke="currentColor"' in svg_tag or "stroke='currentColor'" in svg_tag)
            ):
                return svg_tag
