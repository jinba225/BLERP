...
class CdiscountAdapter(BaseAdapter):
    ...
    def __init__(self, account: "PlatformAccount"):
        ...
        self.base_url = "https://api.cdiscount.com"
        self.api_version = "v1"
        ...
        if not self.seller_id or not self.api_key or not self.secret_key:
            raise ValueError("Cdiscount API配置不完整，需要seller_id、api_key和secret_key")
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            logger.error(f"Cdiscount连接测试失败: {e}")
            return False
    ...
    def _map_product_status(self, status: str) -> str:
        """映射商品状态"""
        if status in ["active", "online", "published"]:
            return "onsale"
        elif status in ["inactive", "offline", "unpublished", "deleted"]:
            return "offshelf"
        return "offshelf"
