...
class AmazonAdapter(BaseAdapter):
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            response = self._make_request('GET', f"/Orders/2013-09-01?signature={signature}", params=params)
    ...
    def get_orders(self, start_date=None, end_date=None, **filters) -> List[Dict]:
        ...
        signature = self._generate_signature(params, '/Orders/2013-09-01')
        response = self._make_request('GET', f"/Orders/2013-09-01", params={**params, 'Signature': signature})
    ...
    def get_products(self, **filters) -> List[Dict]:
        ...
        response = self._make_request('GET', f"/FBAInbound/v0", params={**params, 'Signature': signature})

        inventory = response.get('ListInventorySupplyResult', {}).get('InventorySupplyList', {}).get('member', [])
    ...
    def get_product(self, product_id: str) -> Dict:
        ...
        response = self._make_request('GET', f"/Products/2011-10-01", params={**params, 'Signature': signature})

        product_data = response.get('GetMatchingProductForIdResult', {}).get('Products', {}).get('Product', {})
    ...
    def _parse_products(self, products: List[Dict]) -> List[Dict]:
        ...
        for product in products:
            parsed.append({
                ...
                'status': 'onsale' if int(product.get('TotalSupplyQuantity', 0)) > 0 else 'offshelf',
            })
        return parsed
