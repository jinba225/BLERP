...
class EcommProduct(BaseModel):
    ...
    sync_status = models.CharField(
        "同步状态", max_length=20, choices=SYNC_STATUS_CHOICES, default="pending"
    )
    woo_product_id = models.PositiveIntegerField(
        "WooCommerce产品ID", null=True, blank=True, help_text="WooCommerce中的产品ID"
    )
...
class SyncStrategy(BaseModel):
    ...
    update_fields = models.JSONField(
        "更新字段", default=list, help_text='如：["price", "stock", "status"]'
    )
...
class PlatformAccount(BaseModel):
    ...
    platform = models.ForeignKey(
        Platform, on_delete=models.CASCADE, related_name="accounts", verbose_name="平台"
    )
...
class PlatformOrder(BaseModel):
    ...
    platform = models.ForeignKey(
        Platform, on_delete=models.CASCADE, related_name="orders", verbose_name="平台"
    )
...
class ProductListing(BaseModel):
    ...
    product = models.ForeignKey(
        Product, on_delete=models.CASCADE, related_name="listings", verbose_name="商品"
    )
    platform = models.ForeignKey(
