...
def user_update(request, pk):
    ...
    current_role_ids = UserRole.objects.filter(user=user, is_deleted=False).values_list(
        "role_id", flat=True
    )
...
def role_permissions(request, pk):
            ...
            try:
                ...
                for user_id in selected_users:
                    ...
                    if not UserRole.objects.filter(user_id=user_id, role=role).exists():
                        ...
                        UserRole.objects.create(user=user, role=role, created_by=request.user)
    ...
    current_users = (
        UserRole.objects.filter(role=role).select_related("user").order_by("user__username")
    )
    ...
    available_users = (
        User.objects.filter(is_active=True).exclude(id__in=assigned_user_ids).order_by("username")
    )
