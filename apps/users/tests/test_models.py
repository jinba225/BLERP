...
class RoleModelTest(TestCase):
    ...
    def test_role_unique_name(self):
        ...
        Role.objects.create(name="管理员", code="admin", created_by=self.user)

        # Try to create another role with same name
        with self.assertRaises(Exception):
            Role.objects.create(name="管理员", code="admin2", created_by=self.user)
    ...
    def test_role_unique_code(self):
        ...
        with self.assertRaises(Exception):
            Role.objects.create(name="超级管理员", code="admin", created_by=self.user)

    def test_role_with_permissions(self):
        """Test role with Django permissions."""
        role = Role.objects.create(name="编辑员", code="editor", created_by=self.user)
    ...
    def test_role_str_representation(self):
        ...
        role = Role.objects.create(name="管理员", code="admin", created_by=self.user)
...
class UserRoleModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.role = Role.objects.create(name="管理员", code="admin", created_by=self.admin_user)

    def test_user_role_creation(self):
        """Test user role creation."""
        user_role = UserRole.objects.create(
            user=self.test_user, role=self.role, created_by=self.admin_user
        )
    ...
    def test_user_role_unique_together(self):
        ...
        UserRole.objects.create(user=self.test_user, role=self.role, created_by=self.admin_user)
    ...
    def test_multiple_roles_per_user(self):
        ...
        role2 = Role.objects.create(name="编辑员", code="editor", created_by=self.admin_user)

        ur1 = UserRole.objects.create(
            user=self.test_user, role=self.role, created_by=self.admin_user
        )

        ur2 = UserRole.objects.create(user=self.test_user, role=role2, created_by=self.admin_user)
...
class UserProfileModelTest(TestCase):
    ...
    def test_user_profile_one_to_one(self):
        ...
        profile = UserProfile.objects.create(user=self.user, created_by=self.admin_user)

        # Try to create another profile for same user
        with self.assertRaises(Exception):
            UserProfile.objects.create(user=self.user, created_by=self.admin_user)
...
class LoginLogModelTest(TestCase):
    ...
    def test_login_types(self):
        ...
        for login_type in types:
            log = LoginLog.objects.create(
                user=self.user, login_type=login_type, ip_address="192.168.1.1"
            )
    ...
    def test_session_duration_property(self):
        ...
        # Login with logout
        log1 = LoginLog.objects.create(user=self.user, login_type="web", ip_address="192.168.1.1")
        ...
        log2 = LoginLog.objects.create(user=self.user, login_type="web", ip_address="192.168.1.1")
    ...
    def test_login_log_ordering(self):
        ...
        log2 = LoginLog.objects.create(
            user=self.user, login_type="mobile", ip_address="192.168.1.2"
        )
    ...
    def test_login_log_str_representation(self):
        ...
        log = LoginLog.objects.create(user=self.user, login_type="web", ip_address="192.168.1.1")
