...
class ProductFactory(DjangoModelFactory):
    ...
    sale_price = factory.LazyAttribute(
        lambda obj: obj.cost_price * Decimal('1.3')  # 30%加价
    )
...
class QuoteFactory(DjangoModelFactory):
    ...
    valid_until = factory.LazyAttribute(
        lambda obj: obj.quote_date + timedelta(days=30)
    )
    status = 'draft'
...
class QuoteItemFactory(DjangoModelFactory):
    ...
    line_total = factory.LazyAttribute(
        lambda obj: obj.quantity * obj.unit_price
    )
    discount_amount = Decimal('0.00')
...
class PurchaseOrderFactory(DjangoModelFactory):
    ...
    order_date = FuzzyDate(date.today() - timedelta(days=60), date.today())
    expected_delivery_date = factory.LazyAttribute(
        lambda obj: obj.order_date + timedelta(days=30)
    )
    status = 'draft'
    total_amount = FuzzyDecimal(50000, 500000, 2)
