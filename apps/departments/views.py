...
def department_detail(request, pk):
    ...
    budgets = DepartmentBudget.objects.filter(
        department=department,
        is_deleted=False
    ).order_by('-year', '-quarter', '-month')[:12]
...
def department_tree(request):
    ...
    root_departments = Department.objects.filter(
        is_deleted=False,
        parent__isnull=True
    ).select_related('manager').prefetch_related('children')
...
def position_detail(request, pk):
    ...
    position = get_object_or_404(
        Position.objects.filter(is_deleted=False).select_related('department'),
        pk=pk
    )
...
def budget_list(request):
    ...
    budgets = DepartmentBudget.objects.filter(
        is_deleted=False
    ).select_related('department', 'approved_by')
    ...
    totals = budgets.aggregate(
        total_budget=Sum('personnel_budget') + Sum('operational_budget') +
                    Sum('equipment_budget') + Sum('other_budget'),
        total_actual=Sum('personnel_actual') + Sum('operational_actual') +
                    Sum('equipment_actual') + Sum('other_actual')
    )
