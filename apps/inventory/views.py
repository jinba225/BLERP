...
def stock_detail(request, pk):
    ...
    transactions = (
        InventoryTransaction.objects.filter(
            product=stock.product, warehouse=stock.warehouse, is_deleted=False
        )
...
def transaction_list(request):
    ...
    transactions = (
        InventoryTransaction.objects.filter(is_deleted=False)
        .select_related("product", "warehouse", "location", "operator")
        .order_by("-created_at")
    )
...
def warehouse_list(request):
    ...
    search = request.GET.get("search", "")
    if search:
        warehouses = warehouses.filter(Q(name__icontains=search) | Q(code__icontains=search))
...
def warehouse_detail(request, pk):
    ...
    warehouse = get_object_or_404(
        Warehouse.objects.select_related("manager"), pk=pk, is_deleted=False
    )
...
def adjustment_list(request):
    ...
    adjustments = (
        StockAdjustment.objects.filter(is_deleted=False)
        .select_related("product", "warehouse", "location", "approved_by", "created_by")
        .order_by("-created_at")
    )
...
def transfer_list(request):
    ...
    transfers = (
        ...
        .select_related("from_warehouse", "to_warehouse", "approved_by", "created_by")
    ...
    search = request.GET.get("search", "")
    if search:
        transfers = transfers.filter(Q(transfer_number__icontains=search))
...
def transfer_detail(request, pk):
    ...
    transfer = get_object_or_404(
        StockTransfer.objects.filter(is_deleted=False)
        .select_related("from_warehouse", "to_warehouse", "approved_by", "created_by", "updated_by")
...
def count_list(request):
    ...
    counts = (
        StockCount.objects.filter(is_deleted=False)
        .select_related("warehouse", "supervisor", "created_by")
    ...
    if search:
        counts = counts.filter(
            Q(count_number__icontains=search) | Q(warehouse__name__icontains=search)
        )
...
def count_detail(request, pk):
    ...
    count = get_object_or_404(
        StockCount.objects.filter(is_deleted=False)
        .select_related("warehouse", "supervisor", "created_by", "updated_by")
...
def inbound_list(request):
    ...
    inbounds = (
        InboundOrder.objects.filter(is_deleted=False)
        .select_related("warehouse", "supplier", "created_by", "approved_by")
        .order_by("-created_at")
    )
...
def outbound_list(request):
    ...
    outbounds = (
        OutboundOrder.objects.filter(is_deleted=False)
        .select_related("warehouse", "customer", "created_by", "approved_by")
        .order_by("-created_at")
    )
    ...
    if request.method == "POST":
        ...
        for item in outbound.items.filter(is_deleted=False).select_related("product"):
            ...
            if stock.quantity < item.quantity:
                messages.error(
                    request,
                    f"产品 {item.product.name} 库存不足（当前: {stock.quantity}，需要: {item.quantity}）",
                )
                return redirect("inventory:outbound_detail", pk=pk)
...
def stock_summary_report(request):
    ...
    if search_query:
        stocks = stocks.filter(
            Q(product__name__icontains=search_query) | Q(product__code__icontains=search_query)
        )
    ...
    total_value = sum(
        (stock.quantity * stock.product.cost_price) for stock in stocks if stock.product.cost_price
    )
...
def stock_transaction_report(request):
    ...
    transactions = (
        InventoryTransaction.objects.filter(is_deleted=False)
        .select_related("product", "warehouse", "location", "created_by")
    ...
    # Calculate statistics
    inbound_quantity = transactions.filter(transaction_type="in").aggregate(total=Sum("quantity"))[
        "total"
    ] or Decimal("0")

    outbound_quantity = transactions.filter(transaction_type="out").aggregate(
        total=Sum("quantity")
    )["total"] or Decimal("0")
...
def stock_alert_report(request):
    ...
    low_stocks = (
        InventoryStock.objects.filter(is_deleted=False, quantity__lt=F("product__safety_stock"))
        .select_related("product", "warehouse", "location")
        .order_by("quantity")
    )
...
def inbound_outbound_statistics(request):
    ...
    inbound_by_type = (
        inbound_orders.values("order_type")
        .annotate(count=Count("id"), total_items=Count("items"))
        .order_by("order_type")
    )

    outbound_by_type = (
        outbound_orders.values("order_type")
        .annotate(count=Count("id"), total_items=Count("items"))
        .order_by("order_type")
    )
    ...
    # Daily statistics
    daily_inbound = (
        inbound_orders.annotate(date=TruncDate("order_date"))
        .values("date")
    ...
    daily_outbound = (
        outbound_orders.annotate(date=TruncDate("order_date"))
        .values("date")
        .annotate(count=Count("id"))
        .order_by("date")
    )
    ...
    total_inbound_items = InboundOrderItem.objects.filter(
        inbound_order__in=inbound_orders, is_deleted=False
    ).aggregate(total=Sum("quantity"))["total"] or Decimal("0")

    total_outbound_items = OutboundOrderItem.objects.filter(
        outbound_order__in=outbound_orders, is_deleted=False
    ).aggregate(total=Sum("quantity"))["total"] or Decimal("0")
...
def inventory_order_report(request):
    ...
    if document_type == "inbound":
        ...
        total_items = InboundOrderItem.objects.filter(
            inbound_order__in=orders, is_deleted=False
        ).aggregate(total=Sum("quantity"))["total"] or Decimal("0")
        ...
        total_items = OutboundOrderItem.objects.filter(
            outbound_order__in=orders, is_deleted=False
        ).aggregate(total=Sum("quantity"))["total"] or Decimal("0")
        ...
        total_items = StockTransferItem.objects.filter(
            transfer__in=orders, is_deleted=False
        ...
        if search_query:
            orders = orders.filter(
                Q(count_number__icontains=search_query) | Q(warehouse__name__icontains=search_query)
            )

        # Statistics
        total_orders = orders.count()
        total_items = StockCountItem.objects.filter(count__in=orders, is_deleted=False).count()
            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    warehouse = Warehouse.objects.filter(
                        code=warehouse_code, is_deleted=False
                    ).first()
