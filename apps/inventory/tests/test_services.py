...
class StockTransferServiceTestCase(InventoryServiceTestCaseBase):
    ...
    def test_update_transfer_basic(self):
        ...
        items_data = [
            {'product_id': self.product1.id, 'requested_quantity': Decimal('10.0000'), 'unit_cost': Decimal('5000.00')},
        ]
        transfer = StockTransferService.create_transfer(self.user, transfer_data, items_data)
        ...
        new_items_data = [
            {'product_id': self.product1.id, 'requested_quantity': Decimal('15.0000'), 'unit_cost': Decimal('5000.00')},
            {'product_id': self.product2.id, 'requested_quantity': Decimal('5.0000'), 'unit_cost': Decimal('3000.00')},
        ]

        updated_transfer = StockTransferService.update_transfer(transfer, self.user, update_data, new_items_data)
    ...
    def test_ship_transfer(self):
        ...
        shipped_transfer = StockTransferService.ship_transfer(transfer, self.user, shipped_quantities)
    ...
    def test_receive_transfer(self):
        ...
        received_transfer = StockTransferService.receive_transfer(transfer, self.user, received_quantities)
...
class StockCountServiceTestCase(InventoryServiceTestCaseBase):
    ...
    def test_update_count_basic(self):
        ...
        updated_count = StockCountService.update_count(count, self.user, update_data, new_items_data)
