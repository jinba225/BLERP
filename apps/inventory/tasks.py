...
def update_inventory_status():
    ...
    try:
        ...
        low_stock_products = Product.objects.filter(
            stocks__quantity__lte=F("min_stock"), is_active=True
        ).distinct()
...
def auto_reorder_products():
    ...
    try:
        ...
        reorder_products = Product.objects.filter(
            stocks__quantity__lte=F("reorder_point"), is_active=True
        ).distinct()
        ...
        for product in reorder_products:
            ...
            if total_quantity <= product.reorder_point:
                ...
                if not pending_request:
                    ...
                    )  # specific logic needed here in real app
                    ...
                    if department and system_user:
                        ...
                        reorder_qty = (
                            product.max_stock - total_quantity
                            if product.max_stock > total_quantity
                            else product.reorder_point
                        )
                        if reorder_qty <= 0:
