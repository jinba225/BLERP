...
class Unit(BaseModel):
    ...
    is_active = models.BooleanField("是否启用", default=True)
    is_default = models.BooleanField("是否默认", default=False, help_text="是否作为系统的默认计量单位")
...
class Product(BaseModel):
    ...
    product_type = models.CharField(
        "产品类型", max_length=20, choices=PRODUCT_TYPES, default="finished"
    )
...
class ProductAttribute(BaseModel):
    ...
    attribute_type = models.CharField(
        "属性类型", max_length=20, choices=ATTRIBUTE_TYPES, default="text"
    )
