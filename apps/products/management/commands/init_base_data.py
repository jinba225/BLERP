...
from apps.products.models import ProductCategory, Unit
    ...
    def _create_units(self, force):
        ...
        for unit_data in units_data:
            unit, created = Unit.objects.get_or_create(
                symbol=unit_data["symbol"], defaults=unit_data
            )
    ...
    def _create_warehouses(self, force):
        ...
        for warehouse_data in warehouses_data:
            warehouse, created = Warehouse.objects.get_or_create(
                code=warehouse_data["code"], defaults=warehouse_data
            )

            if created:
                created_count += 1
                self.stdout.write(
                    self.style.SUCCESS(f"  ✓ 创建仓库: {warehouse.name} ({warehouse.code})")
                )
            else:
                skipped_count += 1
                if force:
                    ...
                    warehouse.save()
                    self.stdout.write(
                        self.style.WARNING(f"  ↻ 更新仓库: {warehouse.name} ({warehouse.code})")
                    )
    ...
    def _create_categories(self, force):
        ...
        for category_data in categories_data:
            category, created = ProductCategory.objects.get_or_create(
                code=category_data["code"], defaults=category_data
            )

            if created:
                created_count += 1
                self.stdout.write(
                    self.style.SUCCESS(f"  ✓ 创建分类: {category.name} ({category.code})")
                )
            else:
                skipped_count += 1
                if force:
                    ...
                    category.save()
                    self.stdout.write(
                        self.style.WARNING(f"  ↻ 更新分类: {category.name} ({category.code})")
                    )
