...
class JournalEntry(BaseModel):
    ...
    journal = models.ForeignKey(
        Journal, on_delete=models.CASCADE, related_name="entries", verbose_name="凭证"
    )
...
class SupplierAccount(BaseModel):
    ...
    def aggregate_from_details(self):
        ...
        # 累计负应付（退货）
        total_negative = details.filter(detail_type="return").aggregate(total=models.Sum("amount"))[
            "total"
        ] or Decimal("0")
        ...
        # 已核销金额
        self.paid_amount = details.aggregate(total=models.Sum("allocated_amount"))[
            "total"
        ] or Decimal("0")
...
class Payment(BaseModel):
    ...
    processed_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, verbose_name="处理人"
    )
...
class BudgetLine(BaseModel):
    ...
    budget = models.ForeignKey(
        Budget, on_delete=models.CASCADE, related_name="lines", verbose_name="预算"
    )
    account = models.ForeignKey(Account, on_delete=models.CASCADE, verbose_name="科目")
...
class FinancialReport(BaseModel):
    ...
    total_assets = models.DecimalField(
        "总资产", max_digits=15, decimal_places=2, default=0, null=True, blank=True
    )
    total_liabilities = models.DecimalField(
        "总负债", max_digits=15, decimal_places=2, default=0, null=True, blank=True
    )
    total_equity = models.DecimalField(
        "所有者权益", max_digits=15, decimal_places=2, default=0, null=True, blank=True
    )
    net_profit = models.DecimalField(
        "净利润", max_digits=15, decimal_places=2, default=0, null=True, blank=True
    )

    # 生成信息
    generated_at = models.DateTimeField("生成时间", auto_now_add=True)
    generated_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, verbose_name="生成人"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.is_default:
            ...
            TaxRate.objects.filter(
                tax_type=self.tax_type, is_default=True, is_deleted=False
...
class Expense(BaseModel):
    ...
    payment_method = models.CharField(
        "支付方式", max_length=20, choices=PAYMENT_METHOD, default="personal_advance"
    )
