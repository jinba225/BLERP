...
class TestFinancialReportsE2E:
    ...
    def test_balance_sheet_generation(
        self, test_users
    ):
        """
        测试: 资产负债表生成和验证
        ...
        # 计算收入和费用科目余额（期间发生额）
        revenue_balance = generator.get_account_balance('6001', report_date, start_date=date(2026, 1, 1))
        sales_expense_balance = generator.get_account_balance('6601', report_date, start_date=date(2026, 1, 1))
        admin_expense_balance = generator.get_account_balance('6602', report_date, start_date=date(2026, 1, 1))
        ...
        # 负债总计
        total_liabilities = (
            ap_balance['ending_balance'] +
            payroll_balance['ending_balance']
        )

        # 权益总计（包含本期利润）
        net_income = (
            revenue_balance['credit'] -  # 收入贷方发生额
            sales_expense_balance['debit'] -  # 费用借方发生额
    ...
    def test_income_statement_generation(
        ...
        # 计算费用
        sales_expense_balance = generator.get_account_balance('6601', report_date, start_date=start_date)
        sales_expense = sales_expense_balance['debit']

        admin_expense_balance = generator.get_account_balance('6602', report_date, start_date=start_date)
        ...
        assert abs(calculated_profit - net_profit) < Decimal('0.01'), \
            f"利润计算错误: {calculated_profit} != {net_profit}"

    def test_cash_flow_statement_generation(
        self, test_users
    ):
        """
        测试: 现金流量表生成和验证
        ...
        assert abs(ending_cash - expected_ending_cash) < Decimal('0.01'), \
