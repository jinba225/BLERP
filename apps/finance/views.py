...
def _generate_supplier_account_from_invoice(invoice):
    ...
    try:
        ...
        if (
            hasattr(invoice.supplier, "payment_terms_days")
            and invoice.supplier.payment_terms_days
        ):
            due_date = invoice.invoice_date + timedelta(
                days=invoice.supplier.payment_terms_days
            )
        ...
        account = SupplierAccount.objects.create(
            ...
            and invoice.reference_id
            and PurchaseOrder.objects.filter(id=invoice.reference_id).first()
            or None,
            invoice=invoice,
            ...
            currency=(
                invoice.get("currency", "CNY") if isinstance(invoice, dict) else "CNY"
            ),
        ...
        logger.exception(
            f"Error generating supplier account from invoice {invoice.invoice_number}: {str(e)}"
        )
    ...
    try:
        ...
        for i in range(item_count):
            ...
            try:
                ...
                # 如果出错，删除已创建的发票
                logger.error(f"Invoice item validation failed: {
        ...
        logger.info(
            f"Created {items_created} items for invoice {invoice.invoice_number}"
        )

        # 检查是否至少创建了一条明细
        if items_created == 0:
            logger.warning(
                f"No valid items created for invoice {invoice.invoice_number}, rolling back"
            )
            invoice.delete()
        ...
        logger.exception(
            f"Unexpected error creating invoice from order {order.order_number}: {str(e)}"
        )
...
def account_detail(request, pk):
    ...
    account = get_object_or_404(
        Account.objects.filter(is_deleted=False).select_related("parent"), pk=pk
    )
    ...
    entries = (
        ...
        .order_by("-journal__journal_date")[:50]
    )
...
def journal_list(request):
    ...
    """
    journals = (
        Journal.objects.filter(is_deleted=False)
        .select_related("prepared_by", "reviewed_by", "posted_by")
    ...
    if search:
        journals = journals.filter(
            Q(journal_number__icontains=search) | Q(description__icontains=search)
        )
...
def customer_account_list(request):
    ...
    """
    # 基础查询
    accounts = CustomerAccount.objects.filter(is_deleted=False).select_related(
    ...
    if search:
        accounts = accounts.filter(
            Q(customer__name__icontains=search) | Q(invoice_number__icontains=search)
        )
...
def customer_account_detail(request, pk):
    account = get_object_or_404(
        CustomerAccount.objects.filter(is_deleted=False).select_related(
            "customer", "sales_order"
        ),
        pk=pk,
    )
    ...
    try:
        ...
        expected_balance = (account.invoice_amount - total_paid).quantize(
            Decimal("0.00"), rounding=ROUND_HALF_UP
        )
    ...
    return render(
        request,
        "modules/finance/customer_account_detail.html",
        {
            "account": account,
            "payments": payments,
...
def supplier_account_list(request):
    ...
    if group_by_supplier:
        ...
        supplier_summary = (
            accounts.values("supplier__id", "supplier__name")
            .annotate(
                total_invoice=Sum("invoice_amount"),
                total_paid=Sum("paid_amount"),
                total_balance=Sum("balance"),
        ...
        for account in page_obj:
            account.return_detail_count = account.details.filter(
                detail_type="return"
            ).count()
    ...
    suppliers = Supplier.objects.filter(is_deleted=False, is_approved=True).order_by(
        "name"
    )
...
def customer_account_writeoff(request, pk):
    ...
    if request.method == "POST":
        ...
        if prepay_id:
            prepay = CustomerPrepayment.objects.filter(
                pk=prepay_id, is_deleted=False
            ).first()
            if not prepay:
        ...
        if prepay_id and effective_prepay_amount > 0:
            try:
                payment_numbers.append(
                    generate_unique_payment_number("payment_receipt")
                )
            except Exception as e:
            ...
            with transaction.atomic():
                ...
                if account.sales_order_id:
                    ...
                    order = SalesOrder.objects.filter(
                        pk=account.sales_order_id, is_deleted=False
                    ).first()
                    if order:
                        qs = CustomerAccount.objects.filter(
                            sales_order_id=order.id, is_deleted=False
                        )
                        agg = qs.aggregate(
                            total_invoice=Sum("invoice_amount"),
                            total_paid=Sum("paid_amount"),
                        ...
                        total_paid = (agg["total_paid"] or Decimal("0.00")).quantize(
                            Decimal("0.00"), rounding=ROUND_HALF_UP
                        )
                        total_balance = (
                            agg["total_balance"] or Decimal("0.00")
                        ).quantize(Decimal("0.00"), rounding=ROUND_HALF_UP)

                        # 逾期判定
                        any_overdue = qs.filter(
                            due_date__lt=timezone.now().date(), balance__gt=0
                        ).exists()
    ...
    prepays = CustomerPrepayment.objects.filter(
        customer=account.customer, is_deleted=False, balance__gt=0
    )
    return render(
        request,
        "modules/finance/customer_account_writeoff.html",
        {
            "account": account,
            "prepays": prepays,
...
def supplier_account_writeoff(request, pk):
        ...
        if writeoff_type == "supplier":
            ...
            if use_all_prepays and supplier:
                ...
                if effective_prepay_amount > 0:
                    ...
                    prepay.save()
                    used_prepays.append(
                        {"prepay": prepay, "amount": effective_prepay_amount}
                    )
            ...
            with transaction.atomic():
                ...
                if writeoff_type == "supplier":
                    ...
                    sorted_accounts = sorted(
                        accounts, key=lambda x: x.balance, reverse=True
                    )
    ...
    if writeoff_type == "supplier":
        ...
        if supplier:
            ...
            # 计算预付款总余额
            total_prepay_balance = prepays.aggregate(total=Sum("balance"))[
                "total"
            ] or Decimal("0")
...
def customer_prepayment_list(request):
    ...
    # 基础查询
    prepays = CustomerPrepayment.objects.filter(is_deleted=False).select_related(
        "customer"
    )
    ...
    for item in page_obj:
        ...
        customer_prepays = list(
            prepays.filter(customer_id=customer_id_val).order_by("-received_date")
        )
    ...
    # 获取所有客户（用于筛选下拉框）
    customers = (
        Customer.objects.filter(is_deleted=False, prepayments__is_deleted=False)
        .annotate(prepay_count=Count("prepayments"))
        .filter(prepay_count__gt=0)
        .order_by("name")
    )
...
def customer_prepayment_create(request):
    ...
    return render(
        request,
        "modules/finance/customer_prepayment_form.html",
        {"customers": customers},
    )
...
def supplier_prepayment_list(request):
    ...
    # 基础查询
    prepays = SupplierPrepayment.objects.filter(is_deleted=False).select_related(
        "supplier"
    )
    ...
    if search:
        prepays = prepays.filter(
            Q(supplier__name__icontains=search) | Q(notes__icontains=search)
        )
    ...
    for item in page_obj:
        ...
        supplier_prepays = list(
            prepays.filter(supplier_id=supplier_id_val).order_by("-paid_date")
        )
    ...
    # 获取所有供应商（用于筛选下拉框）
    suppliers = (
        Supplier.objects.filter(is_deleted=False, prepayments__is_deleted=False)
        .annotate(prepay_count=Count("prepayments"))
        .filter(prepay_count__gt=0)
        .order_by("name")
    )
...
def supplier_prepayment_manage(request, supplier_id):
    ...
    total_stats = prepays.aggregate(
        total_amount=Sum("amount"), total_balance=Sum("balance")
    )
...
def supplier_prepayment_create(request):
    ...
    return render(
        request,
        "modules/finance/supplier_prepayment_form.html",
        {"suppliers": suppliers},
    )
...
def supplier_prepayment_delete(request, pk):
    ...
    return render(
        request,
        "modules/finance/supplier_prepayment_confirm_delete.html",
        {
            "prepay": prepay,
...
def customer_prepayment_delete(request, pk):
    ...
    return render(
        request,
        "modules/finance/customer_prepayment_confirm_delete.html",
        {"prepay": prepay},
    )
...
def supplier_prepayment_consolidate(request, supplier_id):
    ...
    prepayments = SupplierPrepayment.objects.filter(
        supplier=supplier, is_deleted=False, status="active", balance__gt=0
    ).order_by("-paid_date")
    ...
    if request.method == "POST":
        ...
        if len(prepayment_ids) < 2:
            ...
            return redirect(
                "finance:supplier_prepayment_consolidate", supplier_id=supplier_id
            )
    ...
    return render(
        request, "modules/finance/supplier_prepayment_consolidate.html", context
    )
...
def customer_prepayment_consolidate(request, customer_id):
    ...
    prepayments = CustomerPrepayment.objects.filter(
        customer=customer, is_deleted=False, status="active", balance__gt=0
    ...
    if request.method == "POST":
        ...
        if len(prepayment_ids) < 2:
            ...
            return redirect(
                "finance:customer_prepayment_consolidate", customer_id=customer_id
            )
    ...
    return render(
        request, "modules/finance/customer_prepayment_consolidate.html", context
    )
...
def generate_supplier_account_from_invoice(request, invoice_id):
    ...
    手动从发票生成应付账款。
    """
    invoice = get_object_or_404(Invoice.objects.filter(is_deleted=False), pk=invoice_id)
    ...
    return render(
        request, "modules/finance/confirm_generate_supplier_account.html", context
    )
...
def payment_list(request):
    ...
    if search:
        payments = payments.filter(
            Q(payment_number__icontains=search) | Q(reference_number__icontains=search)
        )
    ...
    # Calculate totals
    totals = payments.aggregate(total_amount=Sum("amount"))
...
def budget_list(request):
    ...
    """
    budgets = (
        Budget.objects.filter(is_deleted=False)
    ...
    if search:
        budgets = budgets.filter(
            Q(name__icontains=search) | Q(budget_code__icontains=search)
        )
...
def dashboard(request):
    ...
    """
    # Get key metrics
    total_receivables = CustomerAccount.objects.filter(is_deleted=False).aggregate(
        total=Sum("balance")
    )["total"] or Decimal("0")

    total_payables = SupplierAccount.objects.filter(is_deleted=False).aggregate(
        total=Sum("balance")
    )["total"] or Decimal("0")

    # Overdue accounts
    overdue_receivables = CustomerAccount.objects.filter(
        is_deleted=False, due_date__lt=timezone.now().date(), balance__gt=0
    ).count()
    ...
    # Recent journals
    recent_journals = Journal.objects.filter(is_deleted=False).order_by(
        "-journal_date"
    )[:10]

    # Recent payments
    recent_payments = Payment.objects.filter(is_deleted=False).order_by(
        "-payment_date"
    )[:10]
...
def invoice_detail(request, pk):
    ...
    invoice = get_object_or_404(
        Invoice.objects.filter(is_deleted=False)
        .select_related("supplier", "customer", "created_by", "updated_by")
            ...
            for i in range(item_count):
                ...
                if description and quantity and unit_price:
                    InvoiceItem.objects.create(
                        ...
                        tax_rate=Decimal(
                            request.POST.get(f"item_{i}_tax_rate", invoice.tax_rate)
                        ),
                        created_by=request.user,
                        updated_by=request.user,
                    )
...
def invoice_edit(request, pk):
    ...
    """
    invoice = get_object_or_404(Invoice.objects.filter(is_deleted=False), pk=pk)
...
def invoice_verify(request, pk):
    ...
    if request.method == "POST":
        try:
            ...
            # 如果是采购发票，自动生成应付账款
            if invoice.invoice_type == "purchase":
                success, account, error = _generate_supplier_account_from_invoice(
                    invoice
                )
                ...
                if success:
                    ...
                    logger.info(f"Auto-generated supplier account {
                    ...
                    logger.warning(
                        f"Invoice {invoice.invoice_number} already has supplier account"
                    )
                else:
                    # 生成失败，回滚发票状态
                    ...
                    invoice.save()
                    messages.error(
                        request, f"发票认证成功，但生成应付账款失败：{error}"
                    )
                    logger.error(f"Failed to generate supplier account for invoice {
            ...
            logger.exception(
                f"Error verifying invoice {invoice.invoice_number}: {str(e)}"
            )
...
def payment_receipt_list(request):
    ...
    payments = (
        Payment.objects.filter(
            is_deleted=False, payment_type="receipt"
        )  # 只显示收款记录
        .select_related("customer", "processed_by", "created_by")
        .order_by("-created_at")
    )
...
def payment_receipt_detail(request, pk):
    ...
    payment = get_object_or_404(
        Payment.objects.filter(is_deleted=False, payment_type="receipt").select_related(
            "customer", "processed_by", "created_by", "updated_by"
        ),
        pk=pk,
    )
...
def payment_receipt_update(request, pk):
    ...
    payment = get_object_or_404(
        Payment, pk=pk, is_deleted=False, payment_type="receipt"
    )
    ...
    if request.method == "POST":
        try:
            ...
            payment.transaction_reference = request.POST.get(
...
def payment_receipt_delete(request, pk):
    ...
    return render(
        request, "modules/finance/payment_receipt_confirm_delete.html", context
    )
...
def payment_receipt_cancel(request, pk):
    ...
    return render(
        request, "modules/finance/payment_receipt_confirm_cancel.html", context
    )
...
def payment_payment_list(request):
    ...
    payments = (
        Payment.objects.filter(
            is_deleted=False, payment_type="payment"
        )  # 只显示付款记录
        .select_related("supplier", "processed_by", "created_by")
        .order_by("-created_at")
    )
...
def payment_payment_detail(request, pk):
    ...
    payment = get_object_or_404(
        Payment.objects.filter(is_deleted=False, payment_type="payment").select_related(
            "supplier", "processed_by", "created_by", "updated_by"
        ),
        pk=pk,
    )
...
def payment_payment_update(request, pk):
    ...
    payment = get_object_or_404(
        Payment, pk=pk, is_deleted=False, payment_type="payment"
    )
...
def payment_payment_delete(request, pk):
    ...
    return render(
        request, "modules/finance/payment_payment_confirm_delete.html", context
    )
...
def payment_payment_cancel(request, pk):
    ...
    return render(
        request, "modules/finance/payment_payment_confirm_cancel.html", context
    )
    ...
    if account_type == "receivable":
        ...
        accounts = (
            CustomerAccount.objects.filter(
                is_deleted=False, sales_order__isnull=False
            )  # 确保有关联订单
        ...
        # 应付账款
        accounts = (
            SupplierAccount.objects.filter(
                is_deleted=False, purchase_order__isnull=False  # 确保有关联订单
            )
...
def supplier_account_payment_list(request):
    ...
    accounts = (
        SupplierAccount.objects.filter(
            is_deleted=False, supplier__isnull=False
        )  # 只显示供应商应付
        .select_related("supplier", "purchase_order")
        .prefetch_related("details")  # 预加载明细
        .order_by("-created_at")
    )
    ...
    return render(
        request, "modules/finance/supplier_account_payment_list.html", context
    )
...
def supplier_account_payment_allocate(request, pk):
    ...
    account = get_object_or_404(
        SupplierAccount.objects.prefetch_related(
            "details__receipt", "details__return_order"
        ),
        pk=pk,
        is_deleted=False,
    )
                    ...
                    if alloc_amount:
                        ...
                        if alloc_amount > 0:
                            ...
                            if alloc_amount > balance:
                                raise ValueError(
                                    f"明细 {detail.detail_number} 的核销金额不能超过未核销余额 {balance}"
                                )

                            allocation_data.append(
                                {"detail": detail, "amount": alloc_amount}
                            )
    ...
    return render(
        request, "modules/finance/supplier_account_payment_allocate.html", context
    )
        ...
        prepayments_data.append(
            {
                ...
                "paid_date": (
                    prepay.paid_date.strftime("%Y-%m-%d") if prepay.paid_date else ""
                ),
            }
        )
        ...
        prepayments_data.append(
            {
                ...
                "amount": str(prepay.amount),
                "received_date": (
                    prepay.received_date.strftime("%Y-%m-%d")
                    if prepay.received_date
                    else ""
                ),
            }
        )
