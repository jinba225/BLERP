...
class TrendPrediction(BaseModel):
    ...
    platform = models.ForeignKey(Platform, on_delete=models.CASCADE, related_name='trend_predictions', verbose_name='平台')
    platform_account = models.ForeignKey('ecomm_sync.PlatformAccount', on_delete=models.SET_NULL, null=True, blank=True, related_name='trend_predictions', verbose_name='平台账号')
    ...
    confidence = models.DecimalField('置信度', max_digits=5, decimal_places=2, default=0, help_text='0-100百分比')
    
    # 历史数据（用于对比）
    actual_value = models.DecimalField('实际值', max_digits=15, decimal_places=2, null=True, blank=True)
    ...
    def __str__(self):
        return f"{self.get_prediction_type_display()} - {self.platform.name} - {self.prediction_date}"
...
class UserBehaviorAnalysis(BaseModel):
    ...
    platform = models.ForeignKey(Platform, on_delete=models.CASCADE, related_name='user_behavior_analyses', verbose_name='平台')
    platform_account = models.ForeignKey('ecomm_sync.PlatformAccount', on_delete=models.SET_NULL, null=True, blank=True, related_name='user_behavior_analyses', verbose_name='平台账号')
    ...
    product_price = models.DecimalField('商品价格', max_digits=10, decimal_places=2, null=True, blank=True)
    
    # 交易信息
    order_id = models.CharField('订单ID', max_length=200, blank=True)
    order_amount = models.DecimalField('订单金额', max_digits=10, decimal_places=2, null=True, blank=True)
...
class CustomerSegmentation(BaseModel):
    ...
    platform = models.ForeignKey(Platform, on_delete=models.CASCADE, related_name='customer_segmentations', verbose_name='平台')
    ...
    churn_rate = models.DecimalField('流失率', max_digits=5, decimal_places=2, default=0, help_text='百分比')
...
class RealtimeData(BaseModel):
    ...
    platform = models.ForeignKey(Platform, on_delete=models.CASCADE, related_name='realtime_data', verbose_name='平台')
    shop = models.ForeignKey(Shop, on_delete=models.CASCADE, null=True, blank=True, related_name='realtime_data', verbose_name='仓库')
...
class RealtimeWidgetConfig(BaseModel):
    ...
    dashboard = models.ForeignKey(Dashboard, on_delete=models.CASCADE, related_name='widget_configs', verbose_name='仪表盘')
    ...
    # 数据配置
    data_source = models.CharField('数据源', max_length=200, help_text='数据源：如bi.services.realtimedata.RealtimeDataService')
    data_params = models.JSONField('数据参数', default=dict, blank=True)
    data_refresh_interval = models.PositiveIntegerField('刷新间隔(秒)', choices=WIDGET_REFRESH_INTERVALS, default=60)
...
class CustomerLTV(BaseModel):
    ...
    platform = models.ForeignKey(Platform, on_delete=models.CASCADE, related_name='cltv_data', verbose_name='平台')
    customer_email = models.EmailField('客户邮箱', db_index=True)
    ...
    # 预测指标
    predicted_cltv = models.DecimalField('预测CLTV', max_digits=12, decimal_places=2, null=True, blank=True)
    cltv_growth_rate = models.DecimalField('CLTV增长率', max_digits=5, decimal_places=2, null=True, blank=True)
    churn_probability = models.DecimalField('流失概率', max_digits=5, decimal_places=2, null=True, blank=True)
...
class AIAssistant(BaseModel):
    ...
    platform = models.ForeignKey(Platform, on_delete=models.CASCADE, related_name='ai_assistants', verbose_name='平台')
    user = models.ForeignKey('users.User', on_delete=models.CASCADE, related_name='ai_analyses', verbose_name='用户')
...
class ReportTemplate(BaseModel):
    ...
    created_by = models.ForeignKey('users.User', on_delete=models.CASCADE, related_name='created_templates', verbose_name='创建人')
...
class CustomReport(BaseModel):
    ...
    template = models.ForeignKey(ReportTemplate, on_delete=models.SET_NULL, null=True, blank=True, related_name='custom_reports', verbose_name='报表模板')
    ...
    allowed_users = models.ManyToManyField('users.User', blank=True, related_name='accessible_reports', verbose_name='允许访问的用户')
    
    created_by = models.ForeignKey('users.User', on_delete=models.CASCADE, related_name='created_custom_reports', verbose_name='创建人')
...
class CustomReportData(BaseModel):
    ...
    report = models.ForeignKey(CustomReport, on_delete=models.CASCADE, related_name='data_records', verbose_name='报表')
