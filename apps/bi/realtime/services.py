...
from .models import (
    RealtimeData, RealtimeDashboard, RealtimeWidget, RealtimeMetric,
    Dashboard, DashboardWidget
)
from .analytics.services import (
    SalesReportService, InventoryReportService, PlatformComparisonService
)
...
class RealtimeDataService:
    ...
    def update_sales_realtime_data(self, platform_id: int, limit: int = 100) -> RealtimeData:
        ...
        sales_data = queryset.aggregate(
            ...
            paid_orders=Count('id', filter=Q(status='paid') | Q(status='shipped') | Q(status='delivered')),
        ...
        # 获取订单详情
        recent_orders = list(PlatformOrder.objects.filter(
            platform=platform,
    ...
    def update_visitor_realtime_data(self, platform_id: int, limit: int = 100) -> RealtimeData:
        ...
        today = date.today()
        visitor_data = {
            'total_visitors': PlatformOrder.objects.filter(platform=platform, created_at__date=today).count(),
            'unique_visitors': PlatformOrder.objects.filter(platform=platform, created_at__date=today).values('buyer_email').distinct().count(),
    ...
    def update_platform_comparison_realtime_data(self, platform_id: int, limit: int = 100) -> RealtimeData:
        """更新平台对比实时数据"""
        platform = Platform.objects.get(id=platform_id)
        ...
        queryset = PlatformComparison.objects.filter(
            ...
            report_date=today,
            report_period='daily'
        ).select_related('platform').order_by('-sales_amount')
        ...
        for item in queryset[:limit]:
            comparison_data.append({
                ...
                'sales_growth_rate': float(item.sales_growth_rate) if item.sales_growth_rate else 0,
...
class RealtimeDashboardService:
        ...
        for widget in widgets:
            ...
            try:
                widget_config, created = RealtimeWidgetConfig.objects.update_or_create(
                    ...
                    widget=widget,
                    config={
                        'last_updated_at': widget_data['timestamp']
                    }
                )
            except Exception as e:
    ...
    def get_dashboard_summary(self, dashboard_id: int) -> Dict:
        ...
        summary = {
            ...
            'last_updated_at': dashboard.updated_at.strftime('%Y-%m-%d %H:%M') if dashboard.updated_at else '',
            'widgets': [
                {
                    'widget_id': widget.id,
                    'title': widget.title,
