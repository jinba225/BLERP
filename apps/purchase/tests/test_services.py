...
from apps.purchase.services import PurchaseOrderService, PurchaseRequestService
from apps.purchase.models import PurchaseOrder, PurchaseOrderItem, PurchaseRequest, PurchaseRequestItem
...
class PurchaseOrderServiceTestCase(PurchaseServiceTestCaseBase):
    ...
    def test_update_order_basic(self):
        """测试更新采购订单"""
        # 创建订单
        order_data = {'supplier': self.supplier, 'order_date': date.today(), 'status': 'draft', 'warehouse': self.warehouse}
        items_data = [{'product': self.product1, 'quantity': Decimal('1'), 'unit_price': Decimal('5000.00')}]
        order = PurchaseOrderService.create_order(self.user, order_data, items_data)
        ...
        updated_order = PurchaseOrderService.update_order(order, self.user, update_data, new_items_data)
    ...
    def test_update_order_keep_items_when_none(self):
        ...
        updated_order = PurchaseOrderService.update_order(order, self.user, update_data, items_data=None)
...
class PurchaseRequestServiceTestCase(PurchaseServiceTestCaseBase):
    ...
    def test_update_request_basic(self):
        ...
        items_data = [{'product': self.product1, 'quantity': Decimal('1'), 'estimated_price': Decimal('5000.00')}]
        request = PurchaseRequestService.create_request(self.user, request_data, items_data)
        ...
        new_items_data = [
            {'product': self.product1, 'quantity': Decimal('2'), 'estimated_price': Decimal('5000.00')},
            {'product': self.product2, 'quantity': Decimal('3'), 'estimated_price': Decimal('3000.00')},
        ]

        updated_request = PurchaseRequestService.update_request(request, self.user, update_data, new_items_data)
    ...
    def test_convert_request_to_order_basic(self):
        ...
        items_data = [
            {'product': self.product1, 'quantity': Decimal('10'), 'estimated_price': Decimal('5000.00')},
            {'product': self.product2, 'quantity': Decimal('5'), 'estimated_price': Decimal('3000.00')},
        ]
        request = PurchaseRequestService.create_request(self.user, request_data, items_data)
