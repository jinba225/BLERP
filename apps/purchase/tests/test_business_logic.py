...
class OrderApprovalTest(TestCase):
    ...
    def test_order_approval_with_receipt(self):
        ...
        receipt, supplier_account = self.order.approve_order(
        ...
        # Verify supplier account created
        self.assertIsNotNone(supplier_account)
        self.assertEqual(supplier_account.supplier, self.supplier)
        self.assertEqual(supplier_account.purchase_order, self.order)
        self.assertEqual(supplier_account.balance, self.order.total_amount)
    ...
    def test_payment_terms_due_date_calculation(self):
        ...
        receipt, account = order_net30.approve_order(
            approved_by_user=self.approver,
            auto_create_receipt=False
        )

        # 当前实现使用 required_date 作为 due_date
        expected_due_date = order_net30.required_date
...
class OrderUnapprovalTest(TestCase):
    ...
    def setUp(self):
        ...
        self.receipt, self.supplier_account = self.order.approve_order(
            approved_by_user=self.approver,
            warehouse=self.warehouse,
            auto_create_receipt=True
        )
...
class IntegrationTest(TestCase):
    ...
    def test_complete_purchase_flow(self):
        ...
        # 4. Approve order
        receipt, supplier_account = order.approve_order(
