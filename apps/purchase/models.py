...
class PurchaseOrder(BaseModel):
    ...
    sync_to_platform = models.BooleanField("同步到平台", default=False, help_text="是否将采购商品同步到电商平台")
...
class PurchaseRequest(BaseModel):
    ...
    def __str__(self):
        return (
            f"{self.request_number} - {self.requester.get_full_name() if self.requester else 'N/A'}"
        )
    ...
    def approve_and_convert_to_order(
        ...
        if auto_create_order is None:
            config = SystemConfig.objects.filter(
                key="purchase_auto_create_order_on_approve", is_active=True
            ).first()
...
class PurchaseRequestItem(BaseModel):
    ...
    estimated_price = models.DecimalField(
        "预估单价", max_digits=10, decimal_places=2, null=True, blank=True
    )
...
class PurchaseInquiryItem(BaseModel):
    ...
    target_price = models.DecimalField(
        "目标价格", max_digits=10, decimal_places=2, null=True, blank=True
    )
...
class SupplierQuotationItem(BaseModel):
    ...
    unit_price = models.DecimalField(
        "报价单价", max_digits=10, decimal_places=2, validators=[MinValueValidator(0)]
    )
...
class PurchaseReturnItem(BaseModel):
    ...
    def __str__(self):
        return (
            f"{self.purchase_return.return_number} - {self.product.name if self.product else 'N/A'}"
        )
...
class Borrow(BaseModel):
    ...
    status = models.CharField(
        "状态", max_length=20, choices=BORROW_STATUS, default="draft", db_index=True
    )
    ...
    def converted_orders(self):
        ...
        return PurchaseOrder.objects.filter(
            notes__contains=self.borrow_number, is_deleted=False
        ).order_by("created_at")
    ...
    def convert_to_order(self, user, supplier_id, items_with_price, warehouse_id=None):
        ...
        if warehouse_id:
            ...
            target_warehouse = get_object_or_404(
                Warehouse, pk=warehouse_id, is_active=True, is_deleted=False
            )
        else:
            # 如果没有指定仓库，查找第一个主仓
...
class BorrowItem(BaseModel):
    ...
    borrow = models.ForeignKey(
        Borrow, on_delete=models.CASCADE, related_name="items", verbose_name="借用单"
    )
    product = models.ForeignKey("products.Product", on_delete=models.CASCADE, verbose_name="产品")
...
class PurchaseOrderItemPlatformMap(BaseModel):
    ...
    platform_product_id = models.CharField(
        "平台商品ID", max_length=200, db_index=True, help_text="电商平台上的商品ID"
    )
    platform_sku = models.CharField("平台SKU", max_length=200, blank=True, help_text="电商平台上的SKU")
    sync_enabled = models.BooleanField("启用同步", default=True, help_text="是否启用自动同步到电商平台")
    ...
    last_synced_at = models.DateTimeField(
        "最后同步时间", null=True, blank=True, help_text="上次成功同步到电商平台的时间"
    )
    sync_error = models.TextField("同步错误", blank=True, help_text="同步失败时的错误信息")
...
class PurchaseSyncQueue(BaseModel):
    ...
    retry_count = models.IntegerField("重试次数", default=0, help_text="已重试次数")
    max_retries = models.IntegerField("最大重试次数", default=3, help_text="最大重试次数")
    error_message = models.TextField("错误信息", blank=True, help_text="同步失败时的错误信息")
    processed_at = models.DateTimeField("处理时间", null=True, blank=True, help_text="任务开始处理的时间")
    completed_at = models.DateTimeField("完成时间", null=True, blank=True, help_text="任务完成的时间")
