...
from apps.core.models import PAYMENT_METHOD_CHOICES
...
def order_list(request):
    ...
    orders = PurchaseOrder.objects.filter(
        is_deleted=False
    ).select_related('supplier', 'buyer', 'warehouse')
...
def order_detail(request, pk):
    ...
    order = get_object_or_404(
        PurchaseOrder.objects.filter(is_deleted=False).select_related(
            'supplier', 'buyer', 'warehouse', 'approved_by'
    ...
    can_request_payment = (
        order.status in ['approved', 'partial_received', 'fully_received'] and
...
def order_update(request, pk):
    ...
    order = get_object_or_404(
        PurchaseOrder.objects.select_related('supplier', 'buyer', 'warehouse')
        .prefetch_related('items__product'),
        pk=pk,
        is_deleted=False
    )
...
def request_list(request):
    ...
    """
    requests = PurchaseRequest.objects.filter(is_deleted=False).select_related(
        'department', 'requester', 'approved_by'
    ).order_by('-created_at')
...
def request_detail(request, pk):
    ...
    purchase_request = get_object_or_404(
        PurchaseRequest.objects.filter(is_deleted=False).select_related(
            'department', 'requester', 'approved_by', 'converted_order'
...
def request_create(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # 创建采购申请单（草稿状态）
            purchase_request = PurchaseRequestService.create_request(request.user, request_data, items_data)
...
def request_update(request, pk):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            PurchaseRequestService.update_request(purchase_request, request.user, request_data, items_data)
...
def request_convert_to_order(request, pk):
    ...
    if request.method == 'POST':
        try:
            ...
            messages.success(request, f'采购申请单 {purchase_request.request_number} 已成功转换为采购订单 {order.order_number}')
            return redirect('purchase:order_detail', pk=order.pk)
        except Exception as e:
...
def order_approve(request, pk):
    ...
    try:
        ...
        if receipt:
            messages.success(request, f'采购订单 {order.order_number} 审核通过，已自动生成收货单 {receipt.receipt_number}')
        else:
...
def receipt_detail(request, pk):
    ...
    receipt = get_object_or_404(
        PurchaseReceipt.objects.filter(is_deleted=False).select_related(
            ...
            for order_item in order.items.filter(is_deleted=False):
                ...
                if received_qty > 0:
                    ...
                    pending_receipt_items = PurchaseReceiptItem.objects.filter(
                        ...
                        receipt__status__in=['received', 'cancelled']
                    )
...
def receipt_receive(request, pk):
    ...
    try:
        ...
        is_from_borrow = Borrow.objects.filter(
            borrow_number__in=receipt.purchase_order.notes.split(),
            is_deleted=False
        ).exists()
        ...
        if is_from_borrow:
            ...
            if borrow_numbers:
                source_borrow = Borrow.objects.filter(
                    borrow_number=borrow_numbers[0],
                    is_deleted=False
                ).first()

        if is_from_borrow and source_borrow:
            # ========== 借用单转采购订单：创建借用仓→主仓的调拨单据 ==========
            from apps.inventory.models import Warehouse, StockTransfer, StockTransferItem
        ...
        messages.success(request, f'收货单 {receipt.receipt_number} 确认收货成功，共收货 {total_received} 件，库存已更新，应付明细已生成')
        return redirect('purchase:receipt_detail', pk=pk)
    except Exception as e:
...
def order_request_payment(request, pk):
    ...
    try:
        ...
        today_count = SupplierAccount.objects.filter(
            invoice_number__startswith=f'PAY{date_str}',
            is_deleted=False
        ).count()
        ...
        messages.success(request, f'已成功申请付款，生成应付账款记录（编号：{account_number}），应付金额：¥{received_amount:.2f}')
...
def return_detail(request, pk):
    ...
    return_order = get_object_or_404(
        PurchaseReturn.objects.filter(is_deleted=False).select_related(
            'purchase_order__supplier', 'receipt', 'approved_by'
        ).prefetch_related('items__order_item__product'),
        pk=pk
    )
...
def return_approve(request, pk):
    ...
    try:
        ...
        warehouse = None
        first_receipt = return_order.purchase_order.receipts.filter(is_deleted=False).order_by('created_at').first()
...
def return_statistics(request):
    ...
    reason_stats = returns.values('reason').annotate(
        count=Count('id'),
        total_amount=Sum('refund_amount')
    ).order_by('-count')

    # Statistics by status
    status_stats = returns.values('status').annotate(
        count=Count('id')
    )
...
def inquiry_list(request):
    ...
    inquiries = PurchaseInquiry.objects.filter(is_deleted=False).select_related(
        'buyer', 'department', 'warehouse', 'created_by'
...
def inquiry_detail(request, pk):
    ...
    items = inquiry.items.filter(is_deleted=False).select_related(
        'product', 'preferred_supplier'
    ).order_by('sort_order')

    # Get all quotations for this inquiry
    quotations = inquiry.quotations.filter(is_deleted=False).select_related(
        'supplier'
...
def inquiry_send(request, pk):
    ...
    preferred_supplier_ids = inquiry.items.values_list('preferred_supplier_id', flat=True).distinct()
...
def quotation_list(request):
    ...
    quotations = SupplierQuotation.objects.filter(is_deleted=False).select_related(
        'inquiry', 'supplier', 'created_by'
    ).order_by('-created_at')
...
def quotation_detail(request, pk):
    ...
    items = quotation.items.filter(is_deleted=False).select_related(
        'product', 'inquiry_item'
    ).order_by('sort_order')
...
def quotation_compare(request, inquiry_pk):
    ...
    quotations = inquiry.quotations.filter(
        is_deleted=False,
        status='submitted'
    ...
    inquiry_items = inquiry.items.filter(is_deleted=False).select_related('product').order_by('sort_order')
...
def inquiry_create_order(request, pk):
    ...
    context = {
        'inquiry': inquiry,
        'quotation': quotation,
        'items': quotation.items.filter(is_deleted=False).select_related('product') if quotation else inquiry.items.filter(is_deleted=False).select_related('product'),
...
def purchase_order_report(request):
    ...
    if notes:
        queryset = queryset.filter(
            Q(notes__icontains=notes) | Q(remark__icontains=notes)
        )
    ...
    if stats['total_count'] and stats['total_count'] > 0:
        stats['avg_amount'] = stats['total_amount'] / stats['total_count'] if stats['total_amount'] else 0
    else:
        stats['avg_amount'] = 0
...
def borrow_list(request):
    ...
    # 基础查询
    borrows = Borrow.objects.filter(is_deleted=False).select_related(
        'supplier', 'buyer', 'created_by', 'converted_order'
    ).prefetch_related('items').order_by('-created_at')
    ...
    if search_query:
        borrows = borrows.filter(
            Q(borrow_number__icontains=search_query) |
...
def borrow_confirm_all_receipt(request, pk):
    ...
    # 检查是否还有剩余可借用数量
    total_borrowable = sum(item.borrowable_quantity for item in borrow.items.filter(is_deleted=False))
    ...
    if request.method == 'POST':
        ...
        for item in borrow.items.filter(is_deleted=False):
            ...
            if return_qty > item.remaining_quantity:
                messages.error(request, f'产品 {item.product.name} 的归还数量不能超过剩余数量 {item.remaining_quantity}')
...
def borrow_request_conversion(request, pk):
    """直接转采购（无需审核）"""
    from decimal import Decimal
    from apps.core.utils.document_number import DocumentNumberGenerator
    ...
    if request.method == 'POST':
        ...
        for item in borrow.items.filter(is_deleted=False):
            ...
            if convert_qty > item.remaining_quantity:
                messages.error(request, f'产品 {item.product.name} 的转采购数量不能超过剩余数量 {item.remaining_quantity}')
        ...
        try:
            ...
            for data in items_with_price:
                ...
                from decimal import Decimal
                item.conversion_quantity = (item.conversion_quantity or Decimal('0')) + data['convert_qty']
