...
class PurchaseOrderAdmin(ImportExportModelAdmin):
    ...
    list_display = ('order_number', 'supplier', 'order_date', 'status', 'payment_status', 'total_amount', 'approved_by')
    list_filter = ('status', 'payment_status', 'order_date', 'supplier')
    search_fields = ('order_number', 'supplier__name', 'reference_number')
    readonly_fields = ('order_number', 'subtotal', 'tax_amount', 'discount_amount', 'total_amount', 'created_at', 'updated_at', 'approved_at')
    fieldsets = (
        ('基本信息', {
            'fields': ('order_number', 'supplier', 'status', 'payment_status')
        }),
        ('日期信息', {
            'fields': ('order_date', 'required_date', 'promised_date', 'received_date')
        }),
        ('人员信息', {
            'fields': ('buyer', 'approved_by', 'approved_at')
        }),
        ('金额信息', {
            'fields': ('subtotal', 'tax_rate', 'tax_amount', 'discount_rate', 'discount_amount', 'shipping_cost', 'total_amount', 'currency')
        }),
        ('收货信息', {
            'fields': ('warehouse', 'delivery_address', 'delivery_contact', 'delivery_phone')
        }),
        ('其他信息', {
            'fields': ('payment_terms', 'reference_number', 'notes', 'internal_notes')
        }),
        ('系统信息', {
...
class PurchaseReceiptAdmin(ImportExportModelAdmin):
    ...
    list_display = ('receipt_number', 'purchase_order', 'receipt_date', 'warehouse', 'status', 'received_by')
...
class PurchaseReturnAdmin(ImportExportModelAdmin):
    ...
    list_display = ('return_number', 'purchase_order', 'return_date', 'status', 'reason', 'refund_amount')
    ...
    fieldsets = (
        ('基本信息', {
            'fields': ('return_number', 'purchase_order', 'receipt', 'status', 'reason')
        }),
        ('日期信息', {
            'fields': ('return_date', 'shipped_date')
        }),
        ('金额信息', {
...
class SupplierQuotationAdmin(ImportExportModelAdmin):
    ...
    list_display = ('quotation_number', 'inquiry', 'supplier', 'quotation_date', 'status', 'total_amount')
...
class BorrowItemInline(admin.TabularInline):
    model = BorrowItem
    extra = 1
    fields = ('product', 'quantity', 'returned_quantity', 'conversion_quantity', 'conversion_unit_price', 'batch_number', 'specifications')
    readonly_fields = ()
...
class BorrowItemAdmin(admin.ModelAdmin):
    list_display = ('borrow', 'product', 'quantity', 'returned_quantity', 'conversion_quantity', 'batch_number')
