    ...
    def handle(self, *args, **options):
        ...
        if borrow.converted_order_id is None:
            ...
            if not dry_run:
                ...
                borrow.save()
                self.stdout.write(self.style.SUCCESS(f'   ✅ 已设置关联: {borrow.borrow_number} -> {order.order_number}'))
        elif borrow.converted_order_id != order.id:
            self.stdout.write(self.style.WARNING(f'   ⚠️  借用单已关联到其他订单: {borrow.converted_order.order_number}'))
        else:
        ...
        try:
            ...
            main_wh = Warehouse.objects.filter(warehouse_type='main', is_active=True, is_deleted=False).first()
            ...
            if transfer_out.exists() and transfer_in.exists():
                ...
                self.stdout.write(self.style.WARNING(f'   ⚠️  调拨事务不完整 (出库: {transfer_out.count()}, 入库: {transfer_in.count()})'))
        ...
        for receipt in receipts:
            ...
            if receipt_transactions.exists():
                self.stdout.write(self.style.WARNING(f'   ⚠️  发现 {receipt_transactions.count()} 个重复事务'))
        ...
        if dry_run:
            ...
            self.stdout.write(self.style.WARNING('如需实际修复，请运行: python manage.py fix_specific_borrow_order BO260116001 PO260116001'))
        else:
