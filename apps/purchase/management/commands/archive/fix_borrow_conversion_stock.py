    ...
    def handle(self, *args, **options):
        ...
        for borrow in borrows:
            ...
            if transfer_out.exists() and transfer_in.exists():
                self.stdout.write(
                    self.style.SUCCESS(
                        f"    ✅ 转采购调拨事务存在 (出库: {transfer_out.count()}, 入库: {transfer_in.count()})"
                    )
                )
            else:
                self.stdout.write(
                    self.style.WARNING(
                        f"    ⚠️  转采购调拨事务不完整 (出库: {transfer_out.count()}, 入库: {transfer_in.count()})"
                    )
                )
            ...
            for receipt in receipts:
                ...
                if receipt_transactions.exists():
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            f"    ⚠️  收货单 {receipt.receipt_number} 创建了 {receipt_transactions.count()} 个重复事务"
                        )
                    )
