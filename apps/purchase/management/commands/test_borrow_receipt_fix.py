...
from apps.purchase.models import PurchaseOrder, PurchaseReceipt, Borrow
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if borrow_numbers:
            ...
            if source_borrow:
                self.stdout.write(self.style.SUCCESS(f'   ✅ 正确识别借用单: {source_borrow.borrow_number}'))
            else:
        ...
        for receipt in receipts:
            ...
            if transfers.exists():
                self.stdout.write(self.style.SUCCESS(f'     ✅ 有关联调拨单: {transfers.first().transfer_number}'))
            else:
