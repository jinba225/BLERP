...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for borrow in borrows:
            ...
            if wrong_transactions.exists():
                ...
                for t in wrong_transactions:
                    self.stdout.write(f'      - {t.transaction_type}: {t.product.name} | {t.warehouse.name} | 数量: {t.quantity} | ref_type: {t.reference_type}')
        ...
        if dry_run:
            ...
            self.stdout.write(self.style.WARNING('如需实际修复，请运行: python manage.py fix_borrow_receipt_stock_transfer'))
