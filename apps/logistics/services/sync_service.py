...
class LogisticsSyncService:
    ...
    def create_shipping_order(self, platform_order_id: int, logistics_company_id: int) -> ShippingOrder:
        """创建物流订单
    ...
    def get_shipping_statistics(self, start_date, end_date) -> Dict[str, Any]:
        ...
        # 统计各状态订单数
        status_counts = ShippingOrder.objects.filter(
            created_at__range=[start_date, end_date]
        ).values('shipping_status').annotate(
            count=Count('id')
        ...
        # 统计各物流公司订单数
        company_counts = ShippingOrder.objects.filter(
            created_at__range=[start_date, end_date]
        ).values('logistics_company__name').annotate(
            count=Count('id')
        ).order_by('-count')
