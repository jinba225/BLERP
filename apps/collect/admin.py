...
class PlatformAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('基础信息', {
            'fields': ('platform_name', 'platform_code', 'platform_type', 'description')
        }),
        ('API配置', {
            'fields': ('api_key', 'api_secret', 'api_url', 'api_version', 'auth_type')
        }),
        ('OAuth配置', {
...
class ShopAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('基础信息', {
            'fields': ('shop_name', 'shop_code', 'shop_id', 'description')
        }),
        ('所属平台', {
            'fields': ('platform',)
        }),
        ('店铺配置', {
            'fields': ('shop_config', 'currency', 'country')
        }),
        ('API凭证', {
...
class CollectTaskAdmin(admin.ModelAdmin):
    """采集任务管理"""
    list_display = ['task_name', 'platform', 'collect_status', 'land_status', 'collect_num', 'success_num', 'fail_num', 'created_at']
    list_filter = ['collect_status', 'land_status', 'platform', 'collect_platform']
    search_fields = ['task_name']
    readonly_fields = [
        'collect_num', 'success_num', 'fail_num', 'success_rate', 'duration',
        'celery_task_id', 'error_msg', 'started_at', 'completed_at',
        'created_at', 'updated_at', 'created_by', 'updated_by'
    ]
    date_hierarchy = 'created_at'
    fieldsets = (
        ('基础信息', {
            'fields': ('task_name', 'platform', 'collect_platform')
        }),
        ('采集配置', {
            'fields': ('collect_urls',)
        }),
...
class CollectItemAdmin(admin.ModelAdmin):
    """采集子项管理"""
    list_display = ['item_name', 'collect_task', 'collect_status', 'land_status', 'collected_at', 'landed_at']
    list_filter = ['collect_status', 'land_status', 'collect_task']
    search_fields = ['item_name', 'item_sku', 'collect_url']
    readonly_fields = ['collected_at', 'landed_at', 'created_at', 'updated_at', 'created_by', 'updated_by']
    fieldsets = (
        ('基础信息', {
            'fields': ('collect_task', 'product', 'collect_url')
        }),
        ('采集信息', {
...
class FieldMapRuleAdmin(admin.ModelAdmin):
    """字段映射规则管理"""
    list_display = ['collect_platform', 'target_type', 'source_field', 'target_field', 'rule_type', 'sort_order', 'is_active']
    list_filter = ['collect_platform', 'target_type', 'rule_type', 'is_active']
    search_fields = ['source_field', 'target_field']
    list_editable = ['sort_order', 'is_active']
    fieldsets = (
        ('映射配置', {
            'fields': ('collect_platform', 'target_type', 'source_field', 'target_field')
        }),
        ('映射规则', {
            'fields': ('rule_type', 'map_rule', 'sort_order')
        }),
        ('状态', {
...
class PricingRuleAdmin(admin.ModelAdmin):
    """定价规则管理"""
    list_display = ['name', 'platform', 'rule_type', 'markup_percent', 'fixed_price', 'is_active', 'created_at']
    ...
    fieldsets = (
        ...
        ('其他配置', {
            'fields': ('round_method', 'min_price', 'max_price')
        }),
        ('状态', {
            'fields': ('is_active',)
        }),
