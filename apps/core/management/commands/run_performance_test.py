...
from apps.bi.models import SystemHealth
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--iterations", type=int, default=10, help="每个端点的测试迭代次数")
        parser.add_argument(
            "--output",
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f"开始性能测试，测试 {len(api_endpoints)} 个API端点，每个端点测试 {iterations} 次")
        )
        ...
        for endpoint in api_endpoints:
            ...
            for i in range(iterations):
                ...
                try:
                    ...
                    self.stdout.write(
                        f"  第 {i+1} 次测试: {duration:.2f}ms (状态码: {response.status_code})"
                    )
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"  数据库查询次数: {len(connection.queries) if connection.queries else 0}"
                )
            )
        ...
        if test_results["api_tests"]:
            ...
            self.stdout.write(
                f"  最慢的API端点: {slowest_endpoint['endpoint']} ({slowest_endpoint['avg_response_time']:.2f}ms)"
            )

            # 找出最快的API端点
            fastest_endpoint = min(test_results["api_tests"], key=lambda x: x["avg_response_time"])
            self.stdout.write(
                f"  最快的API端点: {fastest_endpoint['endpoint']} ({fastest_endpoint['avg_response_time']:.2f}ms)"
            )
            ...
            for test in test_results["api_tests"]:
                if test["avg_response_time"] > 1000:  # 1秒
                    performance_issues.append(
                        f"API端点 {test['endpoint']} 响应时间过长 ({test['avg_response_time']:.2f}ms)"
                    )
        ...
        try:
            ...
            if test_results["api_tests"]:
                avg_response_times = [
                    test["avg_response_time"] for test in test_results["api_tests"]
                ]
            ...
            SystemHealth.objects.create(
                ...
                db_query_time=Decimal(
                    str(test_results["database_performance"].get("query_time", 0))
                ),
