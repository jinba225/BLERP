        ...
        for url_name, description in pages:
            try:
                ...
                if response.status_code == 200:
                    ...
                    if verbose:
                        self.stdout.write(
                            self.style.SUCCESS(f'✓ {description} ({url_name})')
                        )
                else:
                    fail_count += 1
                    if verbose:
                        self.stdout.write(
                            self.style.WARNING(f'⚠ {description} ({url_name}) - 状态码: {response.status_code}')
                        )
                ...
                if verbose:
                    self.stdout.write(
                        self.style.ERROR(f'✗ {description} ({url_name}) - {str(e)}')
                    )
