...
def api_auth_required(view_func):
    ...
    def _wrapped_view(request, *args, **kwargs):
        ...
        if not request.user.is_authenticated:
            ...
            if not auth_header:
                return JsonResponse(
                    {"error": "未提供认证信息", "detail": "Authentication required"},
                    status=401,
                )
            ...
            # 处理Basic认证
                ...
                try:
                    ...
                    if user:
                        ...
                        return JsonResponse(
                            {"error": "认证失败", "detail": "Invalid credentials"},
                            status=401,
                        )
        ...
        if not request.user.is_active:
            return JsonResponse({"error": "账户未激活", "detail": "Account is inactive"}, status=403)

        # 检查密码是否过期
        if hasattr(request.user, "is_password_expired") and request.user.is_password_expired():
            return JsonResponse({"error": "密码已过期", "detail": "Password has expired"}, status=403)
