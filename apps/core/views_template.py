...
def template_create(request):
    ...
    if request.method == 'POST':
        try:
            ...
            template.layout_config = PrintTemplate.get_default_layout_config(template.template_category)
...
def template_export(request, pk):
    ...
    filename = f"{template.name}_{template.template_category}_{timezone.now().strftime('%Y%m%d')}.json"
