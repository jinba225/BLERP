...
class CustomersAPITestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.category = CustomerCategory.objects.create(
            name="重要客户", code="VIP", discount_rate=0.95, created_by=self.user
        )
...
class CustomerContactsAPITestCase(CustomersAPITestCase):
    ...
    def test_get_customer_contacts_excludes_soft_deleted_contacts(self):
        ...
        self.assertEqual(
            all_contacts.count(),
            2,
            f"Expected 2 contacts before delete, got {all_contacts.count()}",
        )
        ...
        # 验证数据库中只剩下1个未删除的联系人
        remaining_contacts = CustomerContact.objects.filter(
            customer=test_customer, is_deleted=False
        )
        self.assertEqual(
            remaining_contacts.count(),
            1,
            f"Expected 1 contact after delete, got {remaining_contacts.count()}",
        )
