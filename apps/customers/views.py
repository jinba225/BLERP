...
def customer_detail(request, pk):
    ...
    customer = get_object_or_404(
        ...
        .select_related("category", "sales_rep")
...
def customer_update(request, pk):
    ...
    categories = CustomerCategory.objects.filter(is_active=True)
    sales_reps = User.objects.filter(is_active=True)
    contacts = CustomerContact.objects.filter(customer=customer, is_deleted=False).order_by(
        "created_at"
    )
        ...
        try:
            ...
            if is_primary:
                CustomerContact.objects.filter(
                    customer=customer, is_primary=True, is_deleted=False
            ...
            if request.POST.get("from_customer_detail") or request.META.get(
                "HTTP_REFERER", ""
            ).endswith(f"/customers/{customer.pk}/"):
                return redirect("customers:customer_detail", pk=customer.pk)
