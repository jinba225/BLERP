...
class AIModelConfig(BaseModel):
    ...
    api_base = models.CharField('API Base URL', max_length=500, blank=True,
...
class AIConversation(BaseModel):
    ...
    channel_user_id = models.CharField('渠道用户ID', max_length=200,
    ...
    context_summary = models.TextField('上下文摘要', blank=True,
                                      help_text='用于上下文压缩')
...
class AIMessage(BaseModel):
    ...
    conversation = models.ForeignKey(AIConversation, verbose_name='会话',
    ...
    content_type = models.CharField('内容类型', max_length=20,
                                   choices=CONTENT_TYPE_CHOICES, default='text')

    # 工具调用记录
    tool_calls = models.JSONField('工具调用', null=True, blank=True,
                                 help_text='记录AI调用的工具和参数')
    tool_results = models.JSONField('工具结果', null=True, blank=True)

    # 模型信息
    model_config = models.ForeignKey(AIModelConfig, verbose_name='使用的模型',
                                    on_delete=models.SET_NULL,
                                    null=True, blank=True)
    tokens_used = models.IntegerField('消耗Token数', default=0)
    response_time = models.DecimalField('响应时间(秒)', max_digits=10,
...
class AITool(BaseModel):
    ...
    # 参数定义（JSON Schema格式）
    parameters = models.JSONField('参数定义',
                                 help_text='JSON Schema格式定义工具参数')

    # 执行配置
    handler_path = models.CharField('处理器路径', max_length=200,
    ...
    # 权限控制
    required_permissions = models.JSONField('所需权限', default=list,
                                          help_text='用户需要的权限列表')
...
class WeChatConfig(BaseModel):
    ...
    corp_secret = models.CharField('应用Secret', max_length=500,
                                  help_text='将被加密存储')
...
class DingTalkConfig(BaseModel):
    ...
    app_secret = models.CharField('AppSecret', max_length=500,
                                 help_text='将被加密存储')
...
class TelegramConfig(BaseModel):
    ...
    bot_token = models.CharField('Bot Token', max_length=500,
                                help_text='从 @BotFather 获取的Token，将被加密存储')
    bot_username = models.CharField('Bot用户名', max_length=100, blank=True,
                                   help_text='机器人的@username')
    webhook_url = models.CharField('Webhook URL', max_length=500, blank=True,
                                  help_text='Telegram回调地址')

    # 功能开关
    allow_groups = models.BooleanField('允许群组', default=False,
                                      help_text='是否允许在群组中使用')
    command_prefix = models.CharField('命令前缀', max_length=10, default='/',
                                     help_text='命令前缀，默认为/')
...
class AIToolExecutionLog(BaseModel):
    ...
    # 关联会话（可选）
    conversation = models.ForeignKey('AIConversation',
                                    on_delete=models.SET_NULL,
                                    null=True, blank=True,
                                    verbose_name='关联会话',
...
class ChannelUserMapping(BaseModel):
    ...
    # 渠道信息
    channel = models.CharField('渠道', max_length=20, choices=CHANNEL_CHOICES)
    external_user_id = models.CharField('外部用户ID', max_length=200,
                                       help_text='微信OpenID、钉钉UserID或Telegram ChatID')
    external_username = models.CharField('外部用户名', max_length=200, blank=True,
