...
from apps.ai_assistant.providers import BaseAIProvider
...
class ChannelAIService:
    ...
    def _get_user_ai_config(self):
        """获取用户 AI 配置"""
        from apps.ai_assistant.models import CustomerAIConfig
        from apps.customers.models import Customer
        
        # 获取用户关联的客户
        try:
            customer = Customer.objects.filter(
                contact_person=self.user.email,
                is_deleted=False
            ).first()
...
- 不确定的回答，请说明并建议用户咨询相关部门"""
    ...
    def process_message(self, message: IncomingMessage) -> OutgoingMessage:
        ...
        if not self.ai_provider:
            return self.channel_adapter.format_error_message(
                'AI 模型未配置，请联系管理员配置',
                self.channel
            )
    ...
    def _update_conversation_stats(self, session_id: str):
        """更新会话统计"""
        from apps.ai_assistant.services import CustomerAIService
        ...
        try:
            ...
            if len(parts) == 3:
                ...
                # 查找用户映射
                from apps.ai_assistant.models import ChannelUserMapping
                ...
                if mapping:
                    ...
                    from apps.customers.models import Customer
                    customer = Customer.objects.filter(
                        contact_person=mapping.user.email,
                        is_deleted=False
                    ).first()
                    
                    if customer:
                        CustomerAIService.increment_conversation_stats(
                            customer.id,
                            message_count=1
                        )
        except Exception as e:
    ...
    def get_permission_summary(self) -> dict:
        ...
        return CustomerAIService.get_customer_permission_summary(
            self.user_ai_config.customer.id
        ) if self.user_ai_config else {}
