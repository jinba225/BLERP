...
class BedrockProvider(BaseAIProvider):
    ...
    def _build_request_body(self, messages: list, **kwargs) -> Dict:
        ...
        for msg in messages:
            if msg.get("role") == "system":
                bedrock_messages.append(
                    {"role": msg.get("role"), "content": [{"text": msg.get("content")}]}
                )
            elif msg.get("role") == "user":
                bedrock_messages.append({"role": "user", "content": [{"text": msg.get("content")}]})
            elif msg.get("role") == "assistant":
                bedrock_messages.append(
                    {"role": "assistant", "content": [{"text": msg.get("content")}]}
                )
        ...
        return {"messages": bedrock_messages, "inferenceConfig": inference_config}
