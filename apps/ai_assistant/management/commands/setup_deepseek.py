...
from apps.ai_assistant.utils import encrypt_api_key
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not deepseek_api_key:
            self.stdout.write(
                self.style.ERROR(
                    'DEEPSEEK_API_KEY 未在 settings.py 或环境变量中设置'
                )
            )
            self.stdout.write(
                '请在 settings.py 中添加:\n'
        ...
        if existing_config:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f'✓ 已更新 DeepSeek 配置 (ID: {existing_config.id})'
                )
            )
        else:
            # 创建新配置
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f'✓ 已创建 DeepSeek 配置 (ID: {config.id})'
                )
            )
