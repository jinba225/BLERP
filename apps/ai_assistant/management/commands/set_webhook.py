...
class Command(BaseCommand):
    ...
    def _set_telegram_webhook(self, webhook_url):
        ...
        try:
            ...
            if success:
                ...
                self.stdout.write(self.style.ERROR("❌ Telegram Webhook 设置失败"))
        except Exception as e:
            raise CommandError(f"Telegram Webhook 设置失败: {str(e)}")

    def _set_wechat_webhook(self, webhook_url):
        """设置企业微信 Webhook"""
        self.stdout.write(self.style.WARNING("⚠️  企业微信 Webhook 需要在企业微信管理后台手动配置"))
    ...
    def _set_dingtalk_webhook(self, webhook_url):
        """设置钉钉 Webhook"""
        self.stdout.write(self.style.WARNING("⚠️  钉钉 Webhook 需要在钉钉开放平台手动配置"))
        self.stdout.write(f"   Webhook URL: {webhook_url}")
        self.stdout.write(
