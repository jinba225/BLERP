...
class QueryAccountsTool(BaseTool):
    ...
    def execute(
        self, account_type: str = None, keyword: str = None, limit: int = 50, **kwargs
    ) -> ToolResult:
        """执行查询"""
        try:
            # 构建查询
            ...
            if keyword:
                accounts = accounts.filter(Q(name__icontains=keyword) | Q(code__icontains=keyword))
            ...
            return ToolResult(success=True, data=results, message=f"找到 {len(results)} 个会计科目")

        except Exception as e:
            return ToolResult(success=False, error=f"查询会计科目失败: {str(e)}")
    ...
    def execute(
        ...
        **kwargs,
        ...
        try:
            ...
            return ToolResult(success=True, data=results, message=f"找到 {len(results)} 个会计凭证")

        except Exception as e:
            return ToolResult(success=False, error=f"查询会计凭证失败: {str(e)}")
    ...
    def execute(
        ...
        **kwargs,
        ...
        try:
            ...
            return ToolResult(success=True, data=results, message=f"找到 {len(results)} 个客户账")

        except Exception as e:
            return ToolResult(success=False, error=f"查询客户账失败: {str(e)}")
    ...
    def execute(
        ...
        **kwargs,
        ...
        try:
            ...
            return ToolResult(success=True, data=results, message=f"找到 {len(results)} 个供应商账")

        except Exception as e:
            return ToolResult(success=False, error=f"查询供应商账失败: {str(e)}")
        ...
        try:
            ...
            if keyword:
                payments = payments.filter(
                    Q(payment_number__icontains=keyword) | Q(reference_number__icontains=keyword)
                )
            ...
            return ToolResult(success=True, data=results, message=f"找到 {len(results)} 个收付款记录")

        except Exception as e:
            return ToolResult(success=False, error=f"查询收付款记录失败: {str(e)}")
        ...
        **kwargs,
        ...
        try:
            ...
            if not prepayment_type or prepayment_type == "customer":
                ...
                if date_from:
                    customer_prepayments = customer_prepayments.filter(
                        prepayment_date__gte=date_from
                    )
                ...
                if keyword:
                    customer_prepayments = customer_prepayments.filter(
                        prepayment_number__icontains=keyword
                    )

                customer_prepayments = customer_prepayments.select_related("customer").order_by(
                    "-prepayment_date"
                )[:limit]
            ...
            if not prepayment_type or prepayment_type == "supplier":
                ...
                if date_from:
                    supplier_prepayments = supplier_prepayments.filter(
                        prepayment_date__gte=date_from
                    )
                ...
                supplier_prepayments = supplier_prepayments.select_related("supplier").order_by(
                    "-prepayment_date"
                )[:limit]
            ...
            return ToolResult(success=True, data=results, message=f"找到 {len(results)} 个预付款记录")

        except Exception as e:
            return ToolResult(success=False, error=f"查询预付款失败: {str(e)}")
    ...
    def execute(
        ...
        **kwargs,
        ...
        try:
            ...
            expenses = expenses.select_related("applicant", "department").order_by("-expense_date")[
                :limit
            ]
    ...
    def execute(
        ...
        **kwargs,
        ...
        try:
            ...
            return ToolResult(success=True, data=results, message=f"找到 {len(results)} 个发票记录")

        except Exception as e:
            return ToolResult(success=False, error=f"查询发票失败: {str(e)}")
