    ...
    def execute(self, queries: List[Dict[str, Any]], **kwargs) -> ToolResult:
        ...
        try:
            ...
            for query in queries:
                ...
                if not tool:
                    errors.append({"query": query, "error": f"工具 {tool_name} 不存在"})
                    continue
            ...
            return ToolResult(success=False, error=f"批量查询失败: {str(e)}")
...
class BatchApproveTool(BaseTool):
    ...
    def execute(
        ...
        try:
            ...
            return ToolResult(success=False, error=f"批量审核失败: {str(e)}")
        ...
        try:
            ...
            for result in query_results:
                ...
                for item in result_data.get("data", []):
                    flat_data.append({"工具": tool_name, "参数": str(params), **item})

            if not flat_data:
                return ToolResult(success=False, error="没有可导出的数据")
            ...
            return ToolResult(success=False, error=f"批量导出失败: {str(e)}")
    ...
    def execute(self, items: List[Dict[str, Any]], **kwargs) -> ToolResult:
        ...
        try:
            ...
            for item in items:
                ...
                tool = ToolRegistry.get_tool(tool_name, self.user)
                if not tool:
                    errors.append({"item": item, "error": f"工具 {tool_name} 不存在"})
                    continue
            ...
            return ToolResult(success=False, error=f"批量创建失败: {str(e)}")
