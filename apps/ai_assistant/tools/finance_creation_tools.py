...
class CreateJournalTool(BaseTool):
    ...
    def execute(self, journal_type: str, entries: list,
                journal_date: str = None, notes: str = "", **kwargs) -> ToolResult:
        """执行创建凭证"""
        try:
            # 计算借贷合计
            total_debit = 0.0
            ...
            for entry in entries:
                try:
                    ...
                    return ToolResult(
                        success=False,
                        error=f"科目ID {entry['account_id']} 不存在"
                    )
            ...
            with transaction.atomic():
                ...
                journal_date_obj = datetime.strptime(journal_date, '%Y-%m-%d').date() if journal_date else timezone.now().date()
...
class CreatePaymentTool(BaseTool):
    ...
    def execute(self, payment_type: str, amount: float,
                customer_id: int = None, supplier_id: int = None,
                payment_date: str = None, payment_method: str = "bank_transfer",
                reference_number: str = "", notes: str = "", **kwargs) -> ToolResult:
        """执行创建收付款"""
        try:
            amount = float(amount)
...
class CreatePrepaymentTool(BaseTool):
    ...
    def execute(self, prepayment_type: str, amount: float,
                customer_id: int = None, supplier_id: int = None,
                prepayment_date: str = None, payment_method: str = "bank_transfer",
        ...
        try:
            ...
            prepayment_date_obj = datetime.strptime(prepayment_date, '%Y-%m-%d').date() if prepayment_date else timezone.now().date()
...
class CreateExpenseTool(BaseTool):
    ...
    def execute(self, category: str, amount: float,
                expense_date: str = None, department_id: int = None,
                description: str = "", notes: str = "", **kwargs) -> ToolResult:
        """执行创建费用报销"""
        try:
            ...
            if department_id:
                try:
                    ...
                    return ToolResult(
                        success=False,
                        error=f"部门ID {department_id} 不存在"
                    )

            # 创建费用报销
            with transaction.atomic():
                expense_number = DocumentNumberGenerator.generate('expense')
                expense_date_obj = datetime.strptime(expense_date, '%Y-%m-%d').date() if expense_date else timezone.now().date()
