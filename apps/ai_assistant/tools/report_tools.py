...
class GenerateSalesReportTool(BaseTool):
    ...
    def execute(self, date_from: str, date_to: str, group_by: str = "day", **kwargs) -> ToolResult:
        """执行生成报表"""
        try:
            # 验证日期格式
            try:
                ...
                return ToolResult(success=False, error="日期格式错误，应为 YYYY-MM-DD")

            # 查询订单
            orders = SalesOrder.objects.filter(
                order_date__gte=start_date, order_date__lte=end_date, is_deleted=False
            ...
            for status_choice in SalesOrder.STATUS_CHOICES:
                ...
                amount = (
                    orders.filter(status=status).aggregate(total=Sum("total_amount"))["total"] or 0
                )
                status_stats[status] = {
                    "count": count,
                    "amount": float(amount),
            ...
            if group_by == "customer":
                ...
                while current_date <= end_date:
                    ...
                    day_amount = day_orders.aggregate(total=Sum("total_amount"))["total"] or 0
            ...
            return ToolResult(success=False, error=f"生成销售报表失败: {str(e)}")
...
class GeneratePurchaseReportTool(BaseTool):
    ...
    def execute(self, date_from: str, date_to: str, group_by: str = "day", **kwargs) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(success=False, error=f"生成采购报表失败: {str(e)}")
...
class GenerateInventoryReportTool(BaseTool):
    ...
    def execute(
        self, warehouse_id: int = None, low_stock_only: bool = False, **kwargs
    ) -> ToolResult:
        """执行生成报表"""
        try:
            pass
            ...
            # 按产品汇总
            product_stats = inventories.values(
                "product__id", "product__code", "product__name", "product__min_stock"
            ...
            return ToolResult(success=False, error=f"生成库存报表失败: {str(e)}")
