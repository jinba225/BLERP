...
class CreatePurchaseInquiryTool(BaseTool):
    ...
    def execute(self, items: list, supplier_ids: list = None,
                inquiry_date: str = None, required_date: str = "",
        ...
        try:
            ...
            with transaction.atomic():
                ...
                inquiry_date_obj = datetime.strptime(inquiry_date, '%Y-%m-%d').date() if inquiry_date else timezone.now().date()
...
class AddSupplierQuotationTool(BaseTool):
    ...
    def execute(self, inquiry_id: int, supplier_id: int, items: list,
                quote_date: str = None, valid_until: str = "",
                payment_terms: str = "", notes: str = "", **kwargs) -> ToolResult:
        """执行添加报价"""
        try:
            # 获取询价单
            try:
            ...
            with transaction.atomic():
                ...
                quote_date_obj = datetime.strptime(quote_date, '%Y-%m-%d').date() if quote_date else timezone.now().date()
...
class CreatePurchaseReceiptTool(BaseTool):
    ...
    def execute(self, order_id: int, items: list,
                receipt_date: str = None, notes: str = "", **kwargs) -> ToolResult:
        """执行创建收货单"""
        try:
            ...
            with transaction.atomic():
                ...
                receipt_date_obj = datetime.strptime(receipt_date, '%Y-%m-%d').date() if receipt_date else timezone.now().date()
