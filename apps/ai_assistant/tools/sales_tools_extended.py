...
class QueryDeliveriesTool(BaseTool):
    ...
    def execute(self, status: str = None, customer_id: int = None,
        ...
        try:
            ...
            deliveries = deliveries.select_related('sales_order__customer').order_by('-delivery_date')[:limit]
            ...
            for delivery in deliveries:
                results.append({
                    ...
                    "order_number": delivery.sales_order.order_number if delivery.sales_order else "",
                    "customer_name": delivery.sales_order.customer.name if delivery.sales_order else "",
            ...
            return ToolResult(
                success=True,
                data=results,
                message=f"找到 {len(results)} 个发货单"
            )

        except Exception as e:
            return ToolResult(
                success=False,
                error=f"查询发货单失败: {str(e)}"
            )
...
class QuerySalesReturnsTool(BaseTool):
        ...
        return {
            ...
            "properties": {
                "status": {
                    ...
                    "enum": ["pending", "approved", "rejected", "received", "processed", "cancelled"]
                },
                "customer_id": {
                    "type": "integer",
                    "description": "客户ID"
    ...
    def execute(self, status: str = None, customer_id: int = None,
        ...
        try:
            ...
            returns = returns.select_related('sales_order__customer').order_by('-request_date')[:limit]
            ...
            for ret in returns:
                results.append({
                    ...
                    "request_date": ret.request_date.strftime("%Y-%m-%d") if ret.request_date else None,
            ...
            return ToolResult(
                success=True,
                data=results,
                message=f"找到 {len(results)} 个退货单"
            )

        except Exception as e:
            return ToolResult(
                success=False,
                error=f"查询退货单失败: {str(e)}"
            )
    ...
    def execute(self, status: str = None, customer_id: int = None,
        ...
        try:
            ...
            return ToolResult(
                success=True,
                data=results,
                message=f"找到 {len(results)} 个借货单"
            )

        except Exception as e:
            return ToolResult(
                success=False,
                error=f"查询借货单失败: {str(e)}"
            )
    ...
    def execute(self, delivery_id: int, **kwargs) -> ToolResult:
        ...
        try:
            ...
            result = {
                ...
                } if delivery.sales_order else None,
                "customer": {
                    "id": delivery.sales_order.customer.id,
                    "name": delivery.sales_order.customer.name,
                } if delivery.sales_order and delivery.sales_order.customer else None,
                "delivery_date": delivery.delivery_date.strftime("%Y-%m-%d") if delivery.delivery_date else None,
                "tracking_number": delivery.tracking_number or "",
