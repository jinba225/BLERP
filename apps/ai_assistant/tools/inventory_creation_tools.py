...
class CreateWarehouseTool(BaseTool):
    ...
    def execute(self, name: str, code: str, address: str = "",
                manager_id: int = None, is_active: bool = True, **kwargs) -> ToolResult:
        """执行创建仓库"""
        try:
...
class CreateStockTransferTool(BaseTool):
    ...
    def execute(self, source_warehouse_id: int, target_warehouse_id: int, items: list,
                transfer_date: str = None, notes: str = "", **kwargs) -> ToolResult:
        """执行创建调拨单"""
        try:
            # 获取仓库
            try:
            ...
            with transaction.atomic():
                ...
                transfer_date_obj = datetime.strptime(transfer_date, '%Y-%m-%d').date() if transfer_date else timezone.now().date()
...
class CreateStockCountTool(BaseTool):
    ...
    def execute(self, warehouse_id: int, count_date: str = None,
        ...
        try:
            ...
            try:
                ...
                return ToolResult(
                    success=False,
                    error=f"仓库ID {warehouse_id} 不存在"
                )

            # 创建盘点单（不包含明细，后续添加）
            with transaction.atomic():
                count_number = DocumentNumberGenerator.generate('stock_count')
                count_date_obj = datetime.strptime(count_date, '%Y-%m-%d').date() if count_date else timezone.now().date()
...
class CreateInboundOrderTool(BaseTool):
    ...
    def execute(self, warehouse_id: int, items: list,
                inbound_date: str = None, reference_number: str = "",
                notes: str = "", **kwargs) -> ToolResult:
        """执行创建入库单"""
        try:
            # 获取仓库
            ...
            with transaction.atomic():
                ...
                inbound_date_obj = datetime.strptime(inbound_date, '%Y-%m-%d').date() if inbound_date else timezone.now().date()
...
class CreateStockAdjustmentTool(BaseTool):
    ...
    def execute(self, warehouse_id: int, items: list, adjustment_reason: str,
                adjustment_date: str = None, notes: str = "", **kwargs) -> ToolResult:
        """执行创建调整单"""
        try:
            # 获取仓库
            try:
            ...
            adjustment_date_obj = datetime.strptime(adjustment_date, '%Y-%m-%d').date() if adjustment_date else timezone.now().date()
