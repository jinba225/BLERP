...
from inventory.models import (
    Warehouse, InventoryStock, StockTransfer, StockCount,
    InboundOrder, OutboundOrder, StockAdjustment
)
...
class QueryWarehousesTool(BaseTool):
    ...
    def execute(self, is_active: bool = None, keyword: str = None,
        ...
        try:
            ...
            return ToolResult(
                success=True,
                data=results,
                message=f"找到 {len(results)} 个仓库"
            )

        except Exception as e:
            return ToolResult(
                success=False,
                error=f"查询仓库失败: {str(e)}"
            )
...
class QueryStockTransfersTool(BaseTool):
    ...
    def execute(self, status: str = None, source_warehouse_id: int = None,
                target_warehouse_id: int = None, date_from: str = None, date_to: str = None,
        ...
        try:
            ...
            transfers = transfers.select_related('source_warehouse', 'target_warehouse').order_by('-transfer_date')[:limit]
            ...
            for transfer in transfers:
                results.append({
                    ...
                    "source_warehouse": transfer.source_warehouse.name if transfer.source_warehouse else "",
                    "target_warehouse": transfer.target_warehouse.name if transfer.target_warehouse else "",
            ...
            return ToolResult(
                success=True,
                data=results,
                message=f"找到 {len(results)} 个调拨单"
            )

        except Exception as e:
            return ToolResult(
                success=False,
                error=f"查询库存调拨失败: {str(e)}"
            )
...
class QueryStockCountsTool(BaseTool):
    ...
    def execute(self, status: str = None, warehouse_id: int = None,
        ...
        try:
            ...
            for count in counts:
                results.append({
                    ...
                    "count_date": count.count_date.strftime("%Y-%m-%d") if count.count_date else None,
                    "status": count.status,
            ...
            return ToolResult(
                success=True,
                data=results,
                message=f"找到 {len(results)} 个盘点单"
            )

        except Exception as e:
            return ToolResult(
                success=False,
                error=f"查询库存盘点失败: {str(e)}"
            )
    ...
    def execute(self, status: str = None, warehouse_id: int = None,
        ...
        try:
            ...
            return ToolResult(
                success=True,
                data=results,
                message=f"找到 {len(results)} 个入库单"
            )

        except Exception as e:
            return ToolResult(
                success=False,
                error=f"查询入库单失败: {str(e)}"
            )
    ...
    def execute(self, status: str = None, warehouse_id: int = None,
        ...
        try:
            ...
            return ToolResult(
                success=True,
                data=results,
                message=f"找到 {len(results)} 个出库单"
            )

        except Exception as e:
            return ToolResult(
                success=False,
                error=f"查询出库单失败: {str(e)}"
            )
...
class QueryStockAdjustmentsTool(BaseTool):
    ...
    def execute(self, status: str = None, warehouse_id: int = None,
        ...
        try:
            ...
            adjustments = adjustments.select_related('warehouse').order_by('-adjustment_date')[:limit]
            ...
            for adj in adjustments:
                results.append({
                    ...
                    "adjustment_date": adj.adjustment_date.strftime("%Y-%m-%d") if adj.adjustment_date else None,
            ...
            return ToolResult(
                success=True,
                data=results,
                message=f"找到 {len(results)} 个调整单"
            )

        except Exception as e:
            return ToolResult(
                success=False,
                error=f"查询库存调整失败: {str(e)}"
            )
