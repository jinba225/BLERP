...
from purchase.models import PurchaseInquiry, SupplierQuotation, PurchaseReceipt, PurchaseReturn, Borrow
    ...
    def execute(self, status: str = None, supplier_id: int = None,
        ...
        try:
            ...
            for inquiry in inquiries:
                # 获取供应商报价数量
                quotes_count = inquiry.quotes.filter(is_deleted=False).count() if hasattr(inquiry, 'quotes') else 0

                results.append({
                    "id": inquiry.id,
                    "inquiry_number": inquiry.inquiry_number,
                    "inquiry_date": inquiry.inquiry_date.strftime("%Y-%m-%d") if inquiry.inquiry_date else None,
                    "required_date": inquiry.required_date.strftime("%Y-%m-%d") if inquiry.required_date else None,
            ...
            return ToolResult(
                success=True,
                data=results,
                message=f"找到 {len(results)} 个询价单"
            )

        except Exception as e:
            return ToolResult(
                success=False,
                error=f"查询询价单失败: {str(e)}"
            )
...
class QuerySupplierQuotationsTool(BaseTool):
    ...
    def execute(self, status: str = None, supplier_id: int = None,
                inquiry_id: int = None, date_from: str = None, date_to: str = None,
                keyword: str = None, limit: int = 20, **kwargs) -> ToolResult:
        """执行查询"""
        try:
            # 构建查询
            ...
            quotations = quotations.select_related('supplier', 'inquiry').order_by('-quote_date')[:limit]
            ...
            for quot in quotations:
                results.append({
                    ...
                    "quote_date": quot.quote_date.strftime("%Y-%m-%d") if quot.quote_date else None,
                    "valid_until": quot.valid_until.strftime("%Y-%m-%d") if quot.valid_until else None,
                    "status": quot.status,
            ...
            return ToolResult(
                success=True,
                data=results,
                message=f"找到 {len(results)} 个供应商报价"
            )

        except Exception as e:
            return ToolResult(
                success=False,
                error=f"查询供应商报价失败: {str(e)}"
            )
...
class QueryPurchaseReceiptsTool(BaseTool):
    ...
    def execute(self, status: str = None, supplier_id: int = None,
        ...
        try:
            ...
            receipts = receipts.select_related('purchase_order__supplier').order_by('-receipt_date')[:limit]
            ...
            for receipt in receipts:
                results.append({
                    ...
                    "order_number": receipt.purchase_order.order_number if receipt.purchase_order else "",
                    "supplier_name": receipt.purchase_order.supplier.name if receipt.purchase_order and receipt.purchase_order.supplier else "",
            ...
            return ToolResult(
                success=True,
                data=results,
                message=f"找到 {len(results)} 个收货单"
            )

        except Exception as e:
            return ToolResult(
                success=False,
                error=f"查询收货单失败: {str(e)}"
            )
        ...
        try:
            ...
            for ret in returns:
                results.append({
                    ...
                    "return_date": ret.return_date.strftime("%Y-%m-%d") if ret.return_date else None,
            ...
            return ToolResult(
                success=True,
                data=results,
                message=f"找到 {len(results)} 个采购退货单"
            )

        except Exception as e:
            return ToolResult(
                success=False,
                error=f"查询采购退货失败: {str(e)}"
            )
        ...
        return {
            ...
            "properties": {
                "status": {
                    ...
                    "enum": ["pending", "approved", "rejected", "borrowed", "returned", "cancelled"]
                },
                "supplier_id": {
                    "type": "integer",
                    "description": "供应商ID"
        ...
        try:
            ...
            for borrow in borrows:
                results.append({
                    ...
                    "expected_return_date": borrow.expected_return_date.strftime("%Y-%m-%d") if borrow.expected_return_date else None,
            ...
            return ToolResult(
                success=True,
                data=results,
                message=f"找到 {len(results)} 个采购借出单"
            )

        except Exception as e:
            return ToolResult(
                success=False,
                error=f"查询采购借出失败: {str(e)}"
            )
