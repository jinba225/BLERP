...
class ConsolidatePrepaymentTool(BaseTool):
    ...
    def execute(self, prepayment_type: str, source_prepayment_ids: List[int],
                target_prepayment_id: int, notes: str = "", **kwargs) -> ToolResult:
        """执行合并"""
        try:
...
class ProcessPaymentTool(BaseTool):
    ...
    def execute(self, payment_id: int, payment_method: str = "bank_transfer",
                payment_date: str = None, reference_number: str = "",
...
class ApproveBudgetTool(BaseTool):
    ...
    def execute(self, budget_id: int, action: str, notes: str = "",
                new_amount: float = None, **kwargs) -> ToolResult:
        """执行审批预算"""
        try:
