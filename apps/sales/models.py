...
from apps.core.models import BaseModel, PAYMENT_METHOD_CHOICES
...
class SalesOrder(BaseModel):
    ...
    invoice_status = models.CharField('开票状态', max_length=20, choices=INVOICE_STATUS, default='not_invoiced')
    ...
    subtotal = models.DecimalField('含税小计', max_digits=12, decimal_places=2, default=0, help_text='所有明细的含税金额之和')
    tax_rate = models.DecimalField('税率(%)', max_digits=5, decimal_places=2, default=13)
    tax_amount = models.DecimalField('税额', max_digits=12, decimal_places=2, default=0, help_text='从含税价格反推得出')
    ...
    shipping_cost = models.DecimalField('运费', max_digits=12, decimal_places=2, default=0, help_text='含税运费')
    total_amount = models.DecimalField('含税总金额', max_digits=12, decimal_places=2, default=0, help_text='客户实际支付金额（含税）')
    ...
    def calculate_totals(self):
        ...
        if self.tax_rate > 0:
            self.tax_amount = discounted_total / (Decimal('1') + self.tax_rate / Decimal('100')) * (self.tax_rate / Decimal('100'))
        else:
            self.tax_amount = 0
    ...
    def approve_order(self, approved_by_user, warehouse=None, auto_create_delivery=None):
        ...
        if auto_create_delivery:
            ...
            primary_contact = self.customer.contacts.filter(is_primary=True).first() if not self.shipping_contact else None
...
class SalesOrderItem(BaseModel):
    ...
    discount_rate = models.DecimalField('折扣率(%)', max_digits=5, decimal_places=2, default=0)
    discount_amount = models.DecimalField('折扣金额', max_digits=12, decimal_places=2, default=0)
    line_total = models.DecimalField('含税小计', max_digits=12, decimal_places=2, default=0, help_text='含税金额')
...
class Quote(BaseModel):
    ...
    discount_amount = models.DecimalField('折扣金额', max_digits=12, decimal_places=2, default=0)
    total_amount = models.DecimalField('含税总金额', max_digits=12, decimal_places=2, default=0, help_text='客户实际支付金额（含税）')
    total_amount_cny = models.DecimalField('总金额(CNY)', max_digits=12, decimal_places=2, default=0, help_text='自动转换为人民币')
    ...
    def calculate_totals(self):
        ...
        if self.tax_rate > 0:
            self.tax_amount = total_with_tax / (Decimal('1') + self.tax_rate / Decimal('100')) * (self.tax_rate / Decimal('100'))
        else:
            self.tax_amount = 0
    ...
    def is_expired(self):
        ...
        from django.utils import timezone
        return timezone.now().date() > self.valid_until and self.status not in ['converted', 'accepted']
...
class QuoteItem(BaseModel):
    ...
    discount_rate = models.DecimalField('折扣率(%)', max_digits=5, decimal_places=2, default=0)
    discount_amount = models.DecimalField('折扣金额', max_digits=12, decimal_places=2, default=0)
    tax_rate = models.DecimalField('税率(%)', max_digits=5, decimal_places=2, default=13, help_text='增值税税率，默认13%')
    line_total = models.DecimalField('含税小计', max_digits=12, decimal_places=2, default=0, help_text='含税金额')
...
class DeliveryItem(BaseModel):
    ...
    actual_shipped_quantity = models.IntegerField(
        '实际发货数量',
        default=0,
    ...
    def save(self, *args, **kwargs):
        ...
        if self.order_item:
            ...
            if new_total_return_quantity > self.order_item.delivered_quantity:
                available_return_quantity = self.order_item.delivered_quantity - existing_return_quantity
                raise ValueError(f'退货数量 ({self.quantity}) 加上已有退货数量 ({existing_return_quantity}) '
