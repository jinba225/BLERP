...
from apps.core.models import DefaultTemplateMapping, PrintTemplate
...
class SalesAPITestCaseBase(TestCase):
    ...
    def setUp(self):
        ...
        self.category = ProductCategory.objects.create(
            name="电子产品", code="ELEC", created_by=self.user
        )
...
class TemplateAPITestCase(SalesAPITestCaseBase):
    ...
    def test_get_available_templates_quote_domestic(self):
        """测试获取国内报价单模板"""
        url = reverse("sales:api_get_available_templates")
        response = self.client.get(url, {"document_type": "quote", "quote_type": "DOMESTIC"})
    ...
    def test_get_available_templates_quote_overseas(self):
        """测试获取海外报价单模板"""
        url = reverse("sales:api_get_available_templates")
        response = self.client.get(url, {"document_type": "quote", "quote_type": "OVERSEAS"})
    ...
    def test_set_default_template_success(self):
        ...
        response = self.client.post(
            url, {"template_id": self.template2.pk, "document_type": "quote_overseas"}
        )
        ...
        # 验证数据库记录
        mapping = DefaultTemplateMapping.objects.filter(document_type="quote_overseas").first()
    ...
    def test_set_default_template_missing_parameters(self):
        ...
        # 缺少template_id
        response = self.client.post(url, {"document_type": "quote_domestic"})
        ...
        # 缺少document_type
        response = self.client.post(url, {"template_id": self.template1.pk})
        self.assertEqual(response.status_code, 400)
        data = response.json()
        self.assertFalse(data["success"])
    ...
    def test_set_default_template_template_not_found(self):
        """测试模板不存在"""
        url = reverse("sales:api_set_default_template")
        response = self.client.post(url, {"template_id": 99999, "document_type": "quote_domestic"})
    ...
    def test_set_default_template_method_not_allowed(self):
        ...
        response = self.client.get(
            url, {"template_id": self.template1.pk, "document_type": "quote_domestic"}
        )
    ...
    def test_set_default_template_update_existing_mapping(self):
        ...
        url = reverse("sales:api_set_default_template")
        response = self.client.post(
            url, {"template_id": self.template2.pk, "document_type": "quote_domestic"}
        )
        ...
        count = DefaultTemplateMapping.objects.filter(document_type="quote_domestic").count()
