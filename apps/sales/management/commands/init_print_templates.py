...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not PrintTemplate.objects.filter(template_type='quote_domestic', is_deleted=False).exists():
        ...
        if not PrintTemplate.objects.filter(template_type='quote_overseas', is_deleted=False).exists():
