...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for template in templates:
            if template.pk in migration_plan:
                ...
                if template.template_category != category:
                    changes.append(f'  template_category: {template.template_category} → {category}')
                    if not dry_run:
                ...
                for doc_type in default_for:
                    ...
                    if existing_mapping:
                        self.stdout.write(f'  ⚠️  默认映射已存在: {doc_type} → {existing_mapping.template.name}')
                    else:
        ...
        if dry_run:
            ...
            self.stdout.write(self.style.WARNING('💡 执行 python manage.py migrate_template_data 进行实际迁移'))
        else:
