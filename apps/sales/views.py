...
def quote_list(request):
    ...
    """
    quotes = Quote.objects.filter(is_deleted=False).select_related(
        'customer', 'sales_rep'
...
def quote_detail(request, pk):
    ...
    quote = get_object_or_404(
        Quote.objects.filter(is_deleted=False).select_related(
            'customer', 'contact_person', 'sales_rep', 'converted_order'
        ).prefetch_related('items__product'),
        pk=pk
    )

    # 使用新的模板选择服务
    from apps.core.services.template_selector import TemplateSelector

    # 确定完整的单据类型（用于获取默认模板）
    document_type_full = 'quote_domestic' if quote.quote_type.upper() == 'DOMESTIC' else 'quote_overseas'
...
def quote_convert_to_order(request, pk):
    ...
    if request.method == 'POST':
        ...
        form = ConvertToOrderForm(initial={
...
def order_list(request):
    ...
    from apps.sales.models import SalesReturnItem, SalesOrderItem
    ...
    try:
        ...
        return JsonResponse({
            ...
            'customer': {
                ...
                'payment_terms': customer.get_payment_terms_display() if customer.payment_terms else '',
            },
...
def order_approve(request, pk):
    ...
    try:
        ...
        if delivery:
            messages.success(
                request,
                f'订单审核成功!已生成发货单 {delivery.delivery_number}'
            )
        else:
            messages.success(
                request,
...
def delivery_detail(request, pk):
    ...
    delivery = get_object_or_404(
        Delivery.objects.filter(is_deleted=False).select_related(
            'sales_order', 'sales_order__customer', 'warehouse',
            'prepared_by', 'shipped_by'
...
def delivery_update(request, pk):
    ...
    if request.method == 'POST':
        try:
            ...
            delivery.shipping_address = request.POST.get('shipping_address', delivery.shipping_address)
            delivery.shipping_contact = request.POST.get('shipping_contact', delivery.shipping_contact)
...
def delivery_ship(request, pk):
    ...
    if request.method == 'POST':
        try:
            ...
            from decimal import Decimal
            from apps.finance.models import CustomerAccount, Invoice, InvoiceItem
            ...
            for delivery_item in delivery.items.all():
                ...
                discount_factor = (Decimal('100') - (oi.discount_rate or Decimal('0'))) / Decimal('100')
            ...
            if delivery.is_fully_shipped:
                ...
                messages.success(request, f'发货单 {delivery.delivery_number} 部分发货成功，并生成销售发票与应收账款。剩余未发货数量: {delivery.total_unshipped_quantity}')
            return redirect('sales:delivery_detail', pk=pk)
...
def return_detail(request, pk):
    ...
    sales_return = get_object_or_404(
        SalesReturn.objects.filter(is_deleted=False).select_related(
            'sales_order', 'sales_order__customer', 'delivery',
            'approved_by'
        ).prefetch_related('items__order_item__product'),
        pk=pk
    )
        ...
        try:
            ...
            for idx, item_data in enumerate(items_data):
                ...
                if order_item_id and quantity > 0:
                    ...
                    available_return_quantity = order_item.delivered_quantity - existing_return_quantity
                    if quantity > available_return_quantity:
                        raise ValueError(f'产品 {order_item.product.name} 的退货数量 ({quantity}) 不能大于可退数量 ({available_return_quantity})')
            ...
            sales_return.refund_amount = total_refund.quantize(Decimal('0.00'), rounding=ROUND_HALF_UP)
            sales_return.save()
            ...
            from django.contrib.auth import get_user_model
            User = get_user_model()
            managers = User.objects.filter(is_staff=True, is_active=True).exclude(id=request.user.id)
            if order.sales_rep:
    ...
    if request.method == 'POST':
        ...
        try:
            with transaction.atomic():
                ...
                # 自动创建退款应付
                from apps.finance.models import Payment, SupplierAccount
                from apps.core.utils import DocumentNumberGenerator
                from decimal import Decimal
                ...
                if not existing_supplier_account and sales_return.refund_amount and sales_return.refund_amount > 0:
                    # 创建应付账款记录（用于退款给客户）
...
def return_receive(request, pk):
    ...
    if request.method == 'POST':
        try:
            ...
            # Notify sales rep
            if sales_return.sales_order.sales_rep and sales_return.sales_order.sales_rep not in [sales_return.created_by, sales_return.approved_by]:
                _create_return_notification(
                    sales_return,
                    'processed',
...
def return_reject(request, pk):
    ...
    if request.method == 'POST':
        try:
            ...
            # Notify sales rep
            if sales_return.sales_order.sales_rep and sales_return.sales_order.sales_rep != sales_return.created_by:
                _create_return_notification(
                    sales_return,
                    'rejected',
                    sales_return.sales_order.sales_rep,
...
def return_statistics(request):
    ...
    # 准备退货原因图表数据
    reason_labels = [dict(SalesReturn.RETURN_REASONS).get(item['reason'], item['reason']) for item in reason_stats]
    reason_data = [item['count'] for item in reason_stats]

    # 3. 按月统计退货趋势
    monthly_stats = returns.annotate(
    ...
    product_stats = SalesReturnItem.objects.filter(
        return_order__in=returns
    ).values(
        'order_item__product__code',
        'order_item__product__name'
    ).annotate(
        return_count=Count('id'),
        total_quantity=Sum('quantity'),
        total_refund=Sum(F('quantity') * F('unit_price'))
    ).order_by('-return_count')[:10]  # Top 10

    # 5. 按客户统计
    customer_stats = returns.values(
    ...
    orders_with_returns = completed_orders.filter(
        salesreturn__isnull=False,
        salesreturn__is_deleted=False
    ).distinct().count()
...
def sales_order_report(request):
    ...
    # 基础查询集
    queryset = SalesOrder.objects.filter(is_deleted=False).select_related(
        'customer', 'sales_rep', 'created_by'
    ).prefetch_related('items__product')
...
def loan_create(request):
    ...
    from apps.products.models import Product
    from django.contrib.auth import get_user_model
    from apps.inventory.models import Warehouse, InventoryTransaction
    User = get_user_model()
...
def loan_detail(request, pk):
    ...
    loan = get_object_or_404(
        SalesLoan.objects.filter(is_deleted=False).select_related(
            'customer', 'salesperson', 'converted_order',
    ...
    can_return = loan.status in ['loaned', 'partially_returned'] and loan.total_remaining_quantity > 0
