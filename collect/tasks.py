...
def apply_field_map_rules(collect_data: Dict[str, Any], platform_code: str,
                         target_type: str = 'product') -> Dict[str, Any]:
    """
    应用字段映射规则
    ...
    rules = FieldMapRule.objects.filter(
        collect_platform=platform_code,
    ...
    for rule in rules:
        ...
        if rule.rule_type == 'direct':
            ...
            try:
                if rule.map_rule:
                    mapped_data[rule.target_field] = eval(rule.map_rule, {'__builtins__': None}, {'value': source_value})
            except:
                mapped_data[rule.target_field] = source_value
        ...
        for url in collect_urls:
            ...
            try:
                ...
                try:
                    ...
                    if images:
                        downloaded_images = image_downloader.download_images(images, save_to_local=True)
...
def create_listing_task(self, product_id: int, platform_id: int, shop_id: int,
                      pricing_rule_id: int = None, translate: bool = False,
        ...
        if translate:
            try:
                ...
                if product.description:
                    listing_data['description'] = translator.translate(product.description, target_language)
...
@shared_task(bind=True, max_retries=2, retry_backoff=3)
def collect_land_sync_task(self, collect_task_id: int, cross_platform_id: int,
                          cross_shop_id: int, pricing_rule_id: int = None,
                          translate: bool = False, target_language: str = 'en'):
    """
    采集+落地+同步全链路任务
