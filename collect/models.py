...
class CollectTask(BaseModel):
    ...
    collect_platform = models.CharField('采集平台', max_length=16, choices=COLLECT_PLATFORM_CHOICES, db_index=True)
    platform = models.ForeignKey('core.Platform', on_delete=models.PROTECT, related_name='collect_tasks', verbose_name='关联平台配置', db_index=True)
    ...
    # 任务状态
    collect_status = models.CharField('采集状态', max_length=16, choices=COLLECT_STATUS_CHOICES, default='pending', db_index=True)
    land_status = models.CharField('落地状态', max_length=16, choices=LAND_STATUS_CHOICES, default='unland', db_index=True)
    ...
    cross_platform = models.ForeignKey('core.Platform', on_delete=models.SET_NULL, null=True, blank=True, related_name='sync_tasks', verbose_name='目标跨境平台')
    cross_shop = models.ForeignKey('core.Shop', on_delete=models.SET_NULL, null=True, blank=True, related_name='sync_tasks', verbose_name='目标跨境店铺')
...
class CollectItem(BaseModel):
    ...
    collect_task = models.ForeignKey(CollectTask, on_delete=models.CASCADE, related_name='collect_items', verbose_name='关联采集任务')
    product = models.ForeignKey(Product, on_delete=models.SET_NULL, null=True, blank=True, related_name='collect_items', verbose_name='关联产品库产品', db_index=True)
    ...
    # 状态
    collect_status = models.CharField('子项采集状态', max_length=16, choices=COLLECT_STATUS_CHOICES, default='pending')
    land_status = models.CharField('子项落地状态', max_length=16, choices=LAND_STATUS_CHOICES, default='unland')
...
class FieldMapRule(BaseModel):
    ...
    collect_platform = models.CharField('采集平台', max_length=16, choices=COLLECT_PLATFORM_CHOICES, db_index=True)
    target_type = models.CharField('映射目标', max_length=16, choices=TARGET_TYPE_CHOICES, db_index=True)
    ...
    map_rule = models.CharField('映射规则', max_length=256, blank=True, help_text='如: 拼接-精品, price*1.5, length*height*width')
...
class PricingRule(BaseModel):
    ...
    platform = models.ForeignKey(Platform, on_delete=models.CASCADE, related_name='pricing_rules', verbose_name='适用平台')
    
    # 定价规则配置
    markup_percent = models.DecimalField('加成百分比', max_digits=5, decimal_places=2, blank=True, null=True, help_text='如: 50 表示加价50%')
    fixed_price = models.DecimalField('固定价格', max_digits=10, decimal_places=2, blank=True, null=True)
    formula = models.CharField('定价公式', max_length=500, blank=True, help_text='如: cost * 1.5 + shipping_cost')
    
    # 其他配置
    round_method = models.CharField('取整方式', max_length=20, blank=True, choices=(
        ('none', '不取整'),
        ('up', '向上取整'),
        ('down', '向下取整'),
        ('nearest', '四舍五入'),
    ...
    def calculate_price(self, cost_price):
        ...
        if self.rule_type == 'markup':
            price = cost_price * (1 + self.markup_percent / 100) if self.markup_percent else cost_price
