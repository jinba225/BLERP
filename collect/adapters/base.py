...
class BaseCollectAdapter(ABC):
    ...
    def __init__(self, platform_config):
        ...
        if not all([self.api_key, self.api_secret, self.api_url]):
            raise PlatformConfigException(
                self.platform_name,
                "API配置不完整，请检查api_key, api_secret, api_url"
            )
    ...
    def _make_request(self, method: str, url: str, params: Dict[str, Any] = None,
                     json_data: Dict[str, Any] = None, headers: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        发起HTTP请求的通用方法
...
class One688CollectAdapter(BaseCollectAdapter):
    ...
    def collect_item(self, item_url: str) -> Dict[str, Any]:
        ...
        # 6. 提取商品数据
        if "product_get_response" not in response or "offer" not in response["product_get_response"]:
    ...
    def normalize_data(self, raw_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if isinstance(image_list, dict):
            images = [image_list.get(f"img{i}", "") for i in range(1, 10) if f"img{i}" in image_list]
