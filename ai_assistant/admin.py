...
class AIModelConfigAdmin(admin.ModelAdmin):
    ...
    list_display = ['name', 'provider', 'model_name', 'is_active', 'is_default',
                   'priority', 'total_requests', 'last_used_at']
    list_filter = ['provider', 'is_active', 'is_default']
    search_fields = ['name', 'model_name']
    readonly_fields = ['total_requests', 'total_tokens', 'last_used_at',
...
class AIConversationAdmin(admin.ModelAdmin):
    ...
    list_display = ['conversation_id', 'user', 'channel', 'status',
                   'message_count', 'last_message_at']
    list_filter = ['channel', 'status', 'created_at']
    search_fields = ['conversation_id', 'user__username', 'title']
    readonly_fields = ['conversation_id', 'message_count', 'last_message_at',
                      'created_at', 'updated_at']
    ...
    fieldsets = (
        ...
        ('会话状态', {
            'fields': ('status', 'title', 'context_summary')
        }),
        ('统计信息', {
...
class AIMessageAdmin(admin.ModelAdmin):
    ...
    list_display = ['id', 'conversation_link', 'role', 'content_preview',
                   'model_config', 'tokens_used', 'created_at']
    ...
    fieldsets = (
        ('消息信息', {
            'fields': ('conversation', 'role', 'content', 'content_type')
        }),
        ('工具调用', {
...
class AIToolAdmin(admin.ModelAdmin):
    ...
    list_display = ['display_name', 'tool_name', 'category', 'is_active',
                   'requires_approval', 'call_count', 'success_count']
    list_filter = ['category', 'is_active', 'requires_approval']
    search_fields = ['tool_name', 'display_name', 'description']
    readonly_fields = ['call_count', 'success_count', 'last_called_at',
                      'created_at', 'updated_at']

    fieldsets = (
        ('基本信息', {
            'fields': ('tool_name', 'display_name', 'category')
        }),
        ('工具描述', {
            'fields': ('description',)
        }),
        ...
        ('执行配置', {
            'fields': ('handler_path', 'requires_approval')
        }),
        ('权限控制', {
...
class DingTalkConfigAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('基本信息', {
            'fields': ('app_key', 'app_secret', 'agent_id')
        }),
        ('状态管理', {
            'fields': ('is_active',)
        }),
...
class TelegramConfigAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('功能设置', {
            'fields': ('allow_groups', 'command_prefix')
        }),
        ('状态管理', {
            'fields': ('is_active',)
        }),
...
class ChannelUserMappingAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('渠道信息', {
            'fields': ('channel', 'external_user_id', 'external_username')
        }),
        ('系统用户', {
            'fields': ('user',)
        }),
...
class AIToolExecutionLogAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ['tool_name', 'user', 'parameters', 'result',
                      'success', 'error_message', 'executed_at',
                      'execution_time', 'conversation', 'created_at']
    date_hierarchy = 'executed_at'

    fieldsets = (
        ('工具信息', {
            'fields': ('tool_name', 'user', 'conversation')
        }),
        ('执行信息', {
            'fields': ('parameters', 'result', 'success', 'error_message')
        }),
        ('时间统计', {
            'fields': ('executed_at', 'execution_time')
        }),
    )
